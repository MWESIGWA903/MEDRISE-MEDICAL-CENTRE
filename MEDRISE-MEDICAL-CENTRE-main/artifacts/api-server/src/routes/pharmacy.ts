...
router.patch("/pharmacy/stock/:id", async (req, res): Promise<void> => {
  ...
  if (wasOk && isLow) {
    ...
    void createAndBroadcast({
      ...
      body: isOut
        ? `${row.drugName} is now out of stock. Please reorder immediately.`
        : `${row.drugName} has ${row.quantity} ${row.unit} remaining (reorder level: ${row.reorderLevel}).`,
...
router.post("/pharmacy/dispense", async (req, res): Promise<void> => {
  ...
  if (updated.quantity <= updated.reorderLevel) {
    ...
    void createAndBroadcast({
      ...
      body: isOut
        ? `${updated.drugName} is now out of stock. Please reorder immediately.`
        : `${updated.drugName} has ${updated.quantity} ${updated.unit} remaining (reorder level: ${updated.reorderLevel}).`,
      severity: isOut ? "urgent" : "warning",
