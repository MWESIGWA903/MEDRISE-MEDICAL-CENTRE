...
router.get("/billing/stats/summary", async (_req, res): Promise<void> => {
  try {
...
router.post("/billing/append-items", async (req, res): Promise<void> => {
  try {
    const schema = z.object({ patientId: z.number().int(), department: z.string().optional(), items: z.array(ItemSchema).min(1) });
    ...
    const [existing] = await db.select().from(invoicesTable)
      .where(and(eq(invoicesTable.patientId, patientId), or(eq(invoicesTable.status, "unpaid"), eq(invoicesTable.status, "partial"))))
    ...
    if (existing) {
      ...
      const [updated] = await db.update(invoicesTable).set({ totalAmount: String(parseFloat(String(existing.totalAmount)) + added), updatedAt: new Date() }).where(eq(invoicesTable.id, existing.id)).returning();
      inv = updated;
    } else {
