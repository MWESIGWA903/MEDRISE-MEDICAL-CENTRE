...
import { sendLabResultsReadyToPatient } from "../lib/email";
...
router.post("/lab/results", async (req, res): Promise<void> => {
  ...
  if (completedOrder) {
    ...
    void createAndBroadcast({
      ...
      body: parsed.data.interpretation
        ? `Interpretation: ${parsed.data.interpretation}`
        : `Result recorded for order #${parsed.data.labOrderId}.`,
      severity: isCritical ? "urgent" : "info",
      relatedId: completedOrder.id,
    });

    // Email the patient when their lab result is ready
    void (async () => {
      const patient = await db
        .select({ email: patientsTable.email, fullName: patientsTable.fullName })
        .from(patientsTable)
        .where(eq(patientsTable.id, completedOrder.patientId))
        .then((r) => r[0]);
      if (patient?.email) {
