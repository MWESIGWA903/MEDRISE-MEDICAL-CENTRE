...
import { db, pool, patientsTable } from "@workspace/db";
import {
  CreatePatientBody,
  GetPatientParams,
  UpdatePatientParams,
...
router.get("/patients/stats/summary", async (_req, res): Promise<void> => {
  try {
...
router.delete("/patients/:id", async (req, res): Promise<void> => {
  ...
  try {
    ...
    const check = await client.query(`SELECT id, full_name FROM patients WHERE id = $1`, [patientId]);
    if (check.rows.length === 0) {
      await client.query("ROLLBACK");
      res.status(404).json({ error: "Patient not found" });
      return;
    }
    const patientName = check.rows[0].full_name as string;

    // Cascade delete all related records in dependency order (leaf tables first)
