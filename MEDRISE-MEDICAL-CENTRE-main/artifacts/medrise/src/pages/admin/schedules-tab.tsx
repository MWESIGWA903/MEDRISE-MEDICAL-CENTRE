...
export default function SchedulesTab({ adminId }: { adminId?: number }) {
  ...
  const { data: shifts = [], isLoading: isShiftsLoading } = useListShifts(
    { month: filterMonth } as any,
    { query: { enabled: activeTab === 'shifts' } as any },
  );
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: leaveRequests = [], isLoading: isLeaveLoading } = useListLeaveRequests({} as any, {
    query: { enabled: activeTab === 'leave' } as any,
  });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const filteredLeave =
    leaveStatusFilter === 'all'
      ? leaveRequests
      : leaveRequests.filter((l) => l.status === leaveStatusFilter);
    ...
    createLeaveMutation.mutate(
      {
        data: {
          staffId,
          leaveType: String(fd.get('leaveType') || 'annual') as
            | 'annual'
            | 'sick'
            | 'maternity'
            | 'paternity'
  ...
  function handleLeaveDecision(id: number, status: 'approved' | 'rejected') {
    updateLeaveMutation.mutate(
      { id, data: { status, approvedBy: adminId } },
      {
        onSuccess: () => {
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Manage staff shift assignments and leave requests.
          </p>
        </div>
                ...
                <DialogContent className="max-w-md">
                  <DialogHeader>
                    ...
                    <DialogDescription className="sr-only">
                      Assign a work shift to a staff member.
                    </DialogDescription>
                  </DialogHeader>
                  <form onSubmit={handleAddShift} className="space-y-4 pt-2">
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                      ...
                      <Button type="submit" disabled={createShiftMutation.isPending}>
                        {createShiftMutation.isPending ? 'Saving...' : 'Assign Shift'}
                      </Button>
                    </div>
                  </form>
                </DialogContent>
                  ...
                  {shifts.map((s) => (
                    <tr key={s.id}>
                      <td className="px-4 py-3 font-medium text-gray-900">
                        {s.staffName ?? `Staff #${s.staffId}`}
                      </td>
                      <td className="px-4 py-3 capitalize text-gray-600 text-xs">{s.staffRole}</td>
                      <td className="px-4 py-3 text-gray-700">{s.date}</td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-block px-2 py-0.5 rounded text-xs font-medium capitalize ${SHIFT_COLORS[s.shift] ?? 'bg-gray-50 text-gray-600'}`}
                        >
                          {s.shift}
                        </span>
                      </td>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Remove this shift assignment for {s.staffName}?
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-1 flex-wrap">
                            ...
                            <Badge variant="outline" className="text-xs capitalize">
                              {req.leaveType.replace('-', ' ')} Leave
                            </Badge>
                            <Badge
                              variant="outline"
                              className={`text-xs capitalize ${LEAVE_COLORS[req.status] ?? ''}`}
                            >
                              {req.status}
                            </Badge>
                          </div>
                        ...
                        {req.status === 'pending' && (
                          <div className="flex gap-2">
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className="gap-1 h-8 text-red-600 border-red-300 hover:bg-red-50"
