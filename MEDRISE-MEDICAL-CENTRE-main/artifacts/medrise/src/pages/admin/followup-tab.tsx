...
import { Calendar, CheckCircle2, AlertTriangle, Clock, User, Stethoscope } from 'lucide-react';
...
export default function FollowUpTab() {
  ...
  const { data: consultations = [], isLoading } = useListConsultations({} as any, {
    query: {} as any,
  });
  ...
  const stats = {
    ...
    overdue: withFollowUp.filter(
      (c) =>
        c.followUpDate &&
        isPast(parseISO(c.followUpDate)) &&
        !isToday(parseISO(c.followUpDate)) &&
        (c.followUpStatus ?? 'pending') === 'pending',
    ).length,
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm">
          Track consultations requiring follow-up visits — overdue, due today, and upcoming.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {sorted.map((consult) => {
            ...
            return (
              <Card
                key={consult.id}
                className={`border shadow-none hover:shadow-sm transition-shadow ${status === 'attended' ? 'opacity-70' : ''}`}
              >
                <CardContent className="p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <p className="font-semibold text-gray-900 truncate">
                          {consult.patientName ?? `Patient #${consult.patientId}`}
                        </p>
                      </div>
                      {consult.staffName && (
                    ...
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      ...
                      {due && (
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium ${due.color}`}
                        >
                          {due.label}
                        </span>
                      )}
                  ...
                  <div className="space-y-1 mb-3 text-sm text-gray-600">
                    <p>
                      ...
                      {format(parseISO(consult.visitDate), 'dd MMM yyyy')}
                    </p>
                    {consult.followUpDate && (
                      <p>
                        <span className="font-medium text-gray-700">Follow-up due:</span>{' '}
                        {format(parseISO(consult.followUpDate), 'dd MMM yyyy')}
                      </p>
                    )}
                    {consult.diagnosis && (
