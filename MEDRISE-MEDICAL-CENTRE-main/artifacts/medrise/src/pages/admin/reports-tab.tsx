...
import {
  BarChart3, Users, Stethoscope, Calendar, FlaskConical, Pill, DollarSign,
  UserCheck, UserX, Printer, Settings2, Loader2, Save,
} from "lucide-react";
import {
    ...
    upsert(
      { data: { targets: payload } },
      {
        onSuccess: () => {
          ...
          toast({ title: "Targets saved", description: `${period.charAt(0).toUpperCase() + period.slice(1)} targets updated successfully.` });
          onOpenChange(false);
        },
        onError: () => {
          toast({ title: "Error", description: "Failed to save targets. Please try again.", variant: "destructive" });
        },
      }
    );
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure performance targets for the <strong>{PERIOD_LABELS[period].toLowerCase()}</strong> reporting period.
            These targets appear in the Summary Report Table.
          </DialogDescription>
        </DialogHeader>
          ...
          {METRIC_CONFIG.map(m => (
            <div key={m.key} className="space-y-1.5">
              <Label htmlFor={`target-${m.key}`} className="text-sm font-medium text-gray-700">
                {m.label}
                {m.unit && <span className="ml-1 text-gray-400 font-normal text-xs">({m.unit})</span>}
              </Label>
              <Input
                id={`target-${m.key}`}
                type="number"
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button className="flex-1 bg-primary hover:bg-primary/90 text-white" onClick={handleSave} disabled={isPending}>
            {isPending ? (
...
export default function ReportsTab() {
  ...
  const summaryRows = [
    ...
    },
    {
      ...
      target: appointmentsCompletedTarget ?? ((summary?.totalAppointments ?? 0) > 0 ? (summary?.totalAppointments ?? 0) : null),
      isRevenue: false,
    },
    {
  ...
  return (
    <div>
      ...
      <SetTargetsDialog
        ...
        period={period}
        targets={targets.map(t => ({ metricKey: t.metricKey, periodType: t.periodType, targetValue: t.targetValue }))}
      />
        ...
        <>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            ...
            {newPatientsTarget !== null && newPatientsTarget > 0 && (
              <Card className="border border-gray-100">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base font-semibold text-gray-800">New Patient Registrations</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex items-end gap-3 mb-2">
                    <p className="text-3xl font-bold text-gray-900">
                      {Math.round(Math.min(((summary?.newPatients ?? 0) / newPatientsTarget) * 100, 100))}%
                    </p>
                    <p className="text-sm text-gray-500 pb-1">{summary?.newPatients ?? 0} of {newPatientsTarget} target</p>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-3">
                    <div
                      className="bg-blue-500 h-3 rounded-full transition-all"
                      style={{ width: `${Math.min(((summary?.newPatients ?? 0) / newPatientsTarget) * 100, 100)}%` }}
                    />
                  </div>
                </CardContent>
              </Card>
            ...
            {consultationsTarget !== null && consultationsTarget > 0 && (
              <Card className="border border-gray-100">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base font-semibold text-gray-800">Outpatient Consultations</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex items-end gap-3 mb-2">
                    <p className="text-3xl font-bold text-gray-900">
                      {Math.round(Math.min(((summary?.totalConsultations ?? 0) / consultationsTarget) * 100, 100))}%
                    </p>
                    <p className="text-sm text-gray-500 pb-1">{summary?.totalConsultations ?? 0} of {consultationsTarget} target</p>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-3">
                    <div
                      className="bg-teal-500 h-3 rounded-full transition-all"
                      style={{ width: `${Math.min(((summary?.totalConsultations ?? 0) / consultationsTarget) * 100, 100)}%` }}
                    />
                  </div>
                </CardContent>
              </Card>
            ...
            {revenueTarget !== null && revenueTarget > 0 && (
              <Card className="border border-gray-100">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base font-semibold text-gray-800">Revenue Collection</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex items-end gap-3 mb-2">
                    <p className="text-3xl font-bold text-gray-900">
                      {Math.round(Math.min((parseFloat(summary?.totalRevenue ?? "0") / revenueTarget) * 100, 100))}%
                    </p>
                    <p className="text-sm text-gray-500 pb-1">UGX {parseFloat(summary?.totalRevenue ?? "0").toLocaleString()} of {revenueTarget.toLocaleString()}</p>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-3">
                    <div
                      className="bg-green-500 h-3 rounded-full transition-all"
                      style={{ width: `${Math.min((parseFloat(summary?.totalRevenue ?? "0") / revenueTarget) * 100, 100)}%` }}
                    />
                  </div>
                </CardContent>
              </Card>
            ...
            {labTestsTarget !== null && labTestsTarget > 0 && (
              <Card className="border border-gray-100">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base font-semibold text-gray-800">Lab Tests Completed vs Target</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex items-end gap-3 mb-2">
                    <p className="text-3xl font-bold text-gray-900">
                      {Math.round(Math.min(((summary?.completedLabOrders ?? 0) / labTestsTarget) * 100, 100))}%
                    </p>
                    <p className="text-sm text-gray-500 pb-1">{summary?.completedLabOrders ?? 0} of {labTestsTarget} target</p>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-3">
                    <div
                      className="bg-orange-500 h-3 rounded-full transition-all"
                      style={{ width: `${Math.min(((summary?.completedLabOrders ?? 0) / labTestsTarget) * 100, 100)}%` }}
                    />
                  </div>
                </CardContent>
              </Card>
            ...
            {appointmentsCompletedTarget !== null && appointmentsCompletedTarget > 0 && (
              <Card className="border border-gray-100">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base font-semibold text-gray-800">Appointments Completed vs Target</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex items-end gap-3 mb-2">
                    <p className="text-3xl font-bold text-gray-900">
                      {Math.round(Math.min(((summary?.completedAppointments ?? 0) / appointmentsCompletedTarget) * 100, 100))}%
                    </p>
                    <p className="text-sm text-gray-500 pb-1">{summary?.completedAppointments ?? 0} of {appointmentsCompletedTarget} target</p>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-3">
                    <div
                      className="bg-purple-500 h-3 rounded-full transition-all"
                      style={{ width: `${Math.min(((summary?.completedAppointments ?? 0) / appointmentsCompletedTarget) * 100, 100)}%` }}
                    />
                  </div>
                </CardContent>
              </Card>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {summaryRows.map(row => {
                    ...
                    const numericValue = isNotTracked ? null : row.isRevenue ? (row as { numericValue?: number }).numericValue ?? 0 : Number(row.value);
                    const pct = !isNotTracked && numericValue !== null && row.target && row.target > 0 ? (numericValue / row.target) * 100 : null;
                    return (
                      <tr key={row.indicator}>
                        <td className="px-4 py-3 text-gray-700">
                          {row.indicator}
                          {isNotTracked && <span className="ml-2 text-xs text-gray-400">(target only)</span>}
                        </td>
                        ...
                        <td className="px-4 py-3 text-right text-gray-500">
                          {targetsLoading ? (
                            ...
                            row.isRevenue ? row.target.toLocaleString() : row.target
                          ) : "—"}
                        </td>
