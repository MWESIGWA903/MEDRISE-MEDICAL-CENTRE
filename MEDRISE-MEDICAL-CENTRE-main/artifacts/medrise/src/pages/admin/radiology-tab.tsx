...
import {
  Plus, Trash2, CheckCircle2, Clock, ChevronDown, ChevronUp, Printer, ScanLine, Activity,
} from "lucide-react";
import { printImagingReport } from "@/lib/print-utils";

const MODALITIES = [
  "X-Ray", "CT Scan", "MRI", "Ultrasound", "Mammography",
  "Fluoroscopy", "Echocardiography", "Doppler Ultrasound", "Bone Densitometry (DEXA)",
  "Nuclear Medicine Scan", "PET Scan", "Angiography",
];

const BODY_PARTS = [
  "Chest", "Abdomen", "Pelvis", "Head / Brain", "Neck", "Spine (Cervical)",
  "Spine (Thoracic)", "Spine (Lumbar)", "Shoulder", "Elbow", "Wrist / Hand",
  "Hip", "Knee", "Ankle / Foot", "Whole Abdomen & Pelvis", "Upper Limb", "Lower Limb",
  "Obstetric (Pregnancy)", "Renal / Urinary Tract", "Hepatobiliary",
];
...
export default function RadiologyTab({ adminId }: { adminId?: number }) {
  ...
  function handleAddOrder(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!modality) { toast({ title: "Select a modality", variant: "destructive" }); return; }
  ...
  function handleMarkInProgress(orderId: number) {
    updateOrderMutation.mutate({ id: orderId, data: { status: "in-progress" } }, {
      onSuccess: () => { toast({ title: "Marked as In Progress" }); qc.invalidateQueries({ queryKey: getListImagingOrdersQueryKey() }); },
    ...
    <div>
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manage imaging orders, record findings, and print reports.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex gap-3 text-sm">
            <span className="flex items-center gap-1 text-yellow-700">
              <Clock className="h-3.5 w-3.5" /> {orders.filter(o => o.status === "requested").length} pending
            </span>
            <span className="flex items-center gap-1 text-green-700">
              <CheckCircle2 className="h-3.5 w-3.5" /> {orders.filter(o => o.status === "completed").length} done
            </span>
          </div>
          <Dialog open={addOpen} onOpenChange={setAddOpen}>
            ...
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription className="sr-only">Create a new radiology/imaging order for a patient.</DialogDescription>
              </DialogHeader>
              <form onSubmit={handleAddOrder} className="space-y-4 pt-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select name="modality" required className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50">
                      <option value="">Select modality...</option>
                      {MODALITIES.map(m => <option key={m} value={m}>{m}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700 block mb-1">Body Part / Region</label>
                    <select name="bodyPart" className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-1">Clinical Indication</label>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        {uniqueModalities.length > 0 && (
          <div className="flex gap-2 flex-wrap ml-2 border-l border-gray-200 pl-3">
            <button onClick={() => setModalityFilter("all")}
                ...
                <div className="flex items-start justify-between p-4">
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    ...
                    {order.status === "completed" && (
                      <Button size="sm" variant="outline" className="h-8 text-xs gap-1 text-green-700 border-green-200 hover:bg-green-50"
                        onClick={() => setFindingsDialog({
                          orderId: order.id,
                          modality: order.modality,
                          bodyPart: order.bodyPart,
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction className="bg-red-600 hover:bg-red-700 text-white"
                            onClick={() => deleteOrderMutation.mutate({ id: order.id }, {
                              onSuccess: () => { toast({ title: "Deleted" }); qc.invalidateQueries({ queryKey: getListImagingOrdersQueryKey() }); }
                            })}>
                            Delete
                          </AlertDialogAction>
                        </AlertDialogFooter>
                ...
                {expanded === order.id && (
                  <div className="px-4 pb-4 border-t border-gray-50 pt-3">
                    {order.status === "completed" && (order.findings || order.impression) ? (
                      <>
                        <p className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Report</p>
                        {order.findings && (
                        ...
                        {order.reportedByName && (
                          <p className="text-xs text-gray-400 mb-2">Reported by: {order.reportedByName}</p>
                        )}
                        <Button
                          size="sm"
                          variant="outline"
                          className="gap-1.5 text-xs h-8 mt-1 text-indigo-700 border-indigo-200 hover:bg-indigo-50"
                          onClick={() => printImagingReport({
                            ...
                            requestedTime: new Date(String(order.requestedAt)).toLocaleTimeString("en-UG", { hour: "2-digit", minute: "2-digit" }),
                            completedAt: order.completedAt ?? undefined,
                            completedTime: order.completedAt ? new Date(String(order.completedAt)).toLocaleTimeString("en-UG", { hour: "2-digit", minute: "2-digit" }) : undefined,
                      ...
                      <p className="text-xs text-gray-400 italic">
                        {order.status === "completed" ? "No findings recorded." : "Awaiting findings entry."}
                      </p>
                    )}
                    {order.notes && (
                      <p className="text-xs text-gray-500 mt-2"><span className="font-medium">Notes:</span> {order.notes}</p>
                    )}
                  </div>
                )}
              </CardContent>
      ...
      {findingsDialog && (
        <Dialog open={!!findingsDialog} onOpenChange={() => setFindingsDialog(null)}>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {findingsDialog.modality}{findingsDialog.bodyPart ? ` — ${findingsDialog.bodyPart}` : ""} Report
              </DialogTitle>
              <DialogDescription className="sr-only">Enter imaging findings and impression for this order.</DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSaveFindings} className="space-y-4 pt-2">
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-1">Impression / Conclusion</label>
                <Textarea
                  name="impression"
                  rows={3}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <Button type="button" variant="outline" onClick={() => setFindingsDialog(null)}>Cancel</Button>
