...
function printForm(type: 'discharge' | 'referral', entry: QueueEntry) {
  ...
  const vitals = entry.vitalsSnapshot
    ? (() => {
        try {
          return JSON.parse(entry.vitalsSnapshot!);
        } catch {
          return {};
        }
      })()
  ...
  const vitalsHtml = Object.keys(vitals).length
    ? Object.entries(vitals)
        .map(
          ([k, v]) =>
            `<tr><td style="padding:4px 8px;font-weight:600;color:#003087;width:40%;">${k}</td><td style="padding:4px 8px;">${v}</td></tr>`,
        )
        .join('')
...
function printInvestigationRequest(
  type: 'lab' | 'imaging' | 'both',
  entry: QueueEntry,
  overrideTests?: { lab?: string[]; imaging?: string[] },
) {
  const today = new Date().toLocaleDateString('en-UG', {
    year: 'numeric',
    month: 'long',
  ...
  const labRows = labTests.length
    ? labTests
        .map(
          (t, i) =>
            `<tr><td style="width:24px;padding:5px 6px;text-align:center;">${i + 1}</td><td style="padding:5px 8px;">${t}</td><td style="width:90px;padding:5px 6px;border-left:1px solid #e5e7eb;font-size:10px;color:#9ca3af;">Result</td></tr>`,
        )
        .join('')
  ...
  const imagingRows = imagingTests.length
    ? imagingTests
        .map(
          (t, i) =>
            `<tr><td style="width:24px;padding:5px 6px;text-align:center;">${i + 1}</td><td style="padding:5px 8px;">${t}</td><td style="width:90px;padding:5px 6px;border-left:1px solid #e5e7eb;font-size:10px;color:#9ca3af;">Findings</td></tr>`,
        )
        .join('')
  ...
  const sections =
    type === 'lab' ? labSection : type === 'imaging' ? imagingSection : labSection + imagingSection;
...
const PRIORITY_CONFIG: Record<
  TriagePriority,
  { label: string; cardBorder: string; cardBg: string; badge: string; badgeBg: string; dot: string }
> = {
  emergency: {
    label: '🔴 Emergency',
...
export default function QueueTab({ staffId }: { staffId?: number }) {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useListQueue({ date: selectedDate }, { query: {} as any });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    createPatientMutation.mutate(
      ...
      },
      {
        onSuccess: (newPatient) => {
          ...
          toast({
            title: `${newPatient.fullName} registered successfully. Use "Add Patient" to add them to the queue.`,
          });
  ...
  const inConsult = entries
    .filter((e) => e.status === 'in-consultation')
    .sort((a, b) => a.arrivalOrder - b.arrivalOrder);
  const nursing = entries
    .filter((e) => e.status === 'nursing')
    .sort((a, b) => a.arrivalOrder - b.arrivalOrder);
  const theatreEntries = entries
    .filter((e) => e.status === 'theatre')
    .sort((a, b) => a.arrivalOrder - b.arrivalOrder);
  const done = entries
    ...
    addMutation.mutate(
      ...
      },
      {
        onSuccess: (newEntry) => {
          ...
          setPriority('non-urgent');
          setNotes('');
          setAssignStaffId('');
          setReferralSource('home');
          setReferralFacility('');
          setReferralReason('');
          setAddPaymentMethod('');
  ...
  const handleStatusChange = (entry: QueueEntry, newStatus: QueueStatus) => {
    updateMutation.mutate(
      { id: entry.id, data: { status: newStatus as any } },
      {
        onSuccess: () => {
  ...
  const handleSkip = (entry: QueueEntry) => {
    updateMutation.mutate(
      { id: entry.id, data: { status: 'skipped' } },
      {
        onSuccess: () => {
          invalidate();
        },
  ...
  const handleTransfer = (entry: QueueEntry) => {
    if (!transferDept) {
      toast({ title: 'Select a department to transfer to', variant: 'destructive' });
      return;
    }
    updateMutation.mutate(
      {
        ...
        data: {
          ...
          transferNote: transferNote || `Transferred to ${transferDept}`,
        },
      },
      {
        onSuccess: () => {
  ...
  const handleSaveNursing = (
    ...
    data: {
      ...
      specialTx: {
        ...
        ivFluidRate: string;
        ivFluidSite: string;
        ivFluidGivenBy: string;
        bloodTx: boolean;
        bloodType: string;
        bloodUnits: string;
        bloodReactions: string;
        bloodGivenBy: string;
    ...
    if (s.ivFluids)
      specialLines.push(
        `  IV Fluids: ${s.ivFluidType || '—'} ${s.ivFluidVol ? s.ivFluidVol + 'ml' : ''} @ ${s.ivFluidRate ? s.ivFluidRate + 'ml/hr' : '—'} | Site: ${s.ivFluidSite || '—'} | By: ${s.ivFluidGivenBy || '—'}`,
      );
    if (s.bloodTx)
      specialLines.push(
        `  Blood Transfusion: ${s.bloodType || '—'}, ${s.bloodUnits || '—'} unit(s) | Reactions: ${s.bloodReactions || 'None'} | By: ${s.bloodGivenBy || '—'}`,
      );
    if (s.woundDressing)
      specialLines.push(
        `  Wound Dressing: Site=${s.woundSite || '—'}, Dressing=${s.dressingType || '—'} | ${s.woundDesc || ''} | By: ${s.woundDoneBy || '—'}`,
      );
    if (s.oxygenTx)
      specialLines.push(
        `  Oxygen Therapy: ${s.o2Device || '—'} @ ${s.o2Rate || '—'} L/min | Target SpO2: ${s.o2SpO2Target || '—'} | By: ${s.o2GivenBy || '—'}`,
      );
    if (s.physio)
      specialLines.push(
        `  Physiotherapy: ${s.physioType || '—'} ${s.physioDuration ? s.physioDuration + 'min' : ''} | Response: ${s.physioResponse || '—'} | By: ${s.physioDoneBy || '—'}`,
      );
    if (s.immunisation)
      specialLines.push(
        `  Immunisation: ${s.vaccineName || '—'} ${s.vaccineDose || ''} | Site: ${s.vaccineSite || '—'} | Batch: ${s.vaccineBatch || '—'} | Next: ${s.vaccineNextDue || '—'} | By: ${s.vaccineGivenBy || '—'}`,
      );
    const parts = [
      ...
      (data.fluidIn || data.fluidOut) &&
        `Fluid Balance: In=${data.fluidIn || '—'} ml / Out=${data.fluidOut || '—'} ml`,
    ]
      .filter(Boolean)
      .join('\n\n');
    if (!parts) {
    ...
    updateMutation.mutate(
      { id: entry.id, data: { notes: existingNotes + separator + parts } },
      {
        onSuccess: async () => {
          ...
          if (entry.patientId && data.specialTx) {
            ...
            for (const key of keys) {
              if (data.specialTx[key]) {
                const overridePrice =
                  data.nursingPriceOverrides?.[key] ?? NURSING_TX_PRICES[key] ?? 0;
                txItems.push({
                  description: NURSING_TX_LABELS[key] || key,
                  quantity: 1,
            ...
            if (txItems.length > 0) {
              try {
                ...
                /* silent — billing can be corrected manually */
              }
            }
          }
        },
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            First-come, first-served — urgent/emergency cases take priority automatically.
          </p>
        </div>
                ...
                <div className="max-h-72 overflow-y-auto divide-y divide-gray-50">
                  {notifications.length === 0 ? (
                    <p className="text-xs text-gray-400 text-center py-6">
                      No alerts yet. Alerts are logged when patients are called or results are
                      ready.
                    </p>
                  ) : (
                    notifications.map((n) => (
                      <div
                        key={n.id}
                        className={`p-3 ${n.type === 'emergency' ? 'bg-red-50' : n.type === 'results' ? 'bg-green-50' : 'bg-white'}`}
                      >
            ...
            <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription className="text-xs text-gray-500">
                  Enter patient details to create a new record. After registration, use "Add
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      ...
                      <Input
                        placeholder="e.g. Mwesigwa Hannington"
                        value={regName}
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="flex-1">
                          <Input
                            ...
                            max={11}
                            value={regAgeMonths}
                            onChange={(e) => setRegAgeMonths(e.target.value)}
                          />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      ...
                      <Select
                        value={regPaymentMethod || 'none'}
                        onValueChange={(v) => setRegPaymentMethod(v === 'none' ? '' : v)}
                      >
                        <SelectTrigger>
                    ...
                    <div>
                      ...
                      <Input
                        placeholder="e.g. NHIS, AAR, Jubilee"
                    ...
                    <div>
                      ...
                      <Input
                        placeholder="e.g. NHIS-12345"
                        value={regInsurancePolicy}
                        onChange={(e) => setRegInsurancePolicy(e.target.value)}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  return (
                    <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600">
                      ...
                      <span className="text-gray-600">
                        {cadreLabel[regStaff.role ?? ''] ?? regStaff.role ?? 'Staff'}
                      </span>
                    </div>
                  );
                })()}
              ...
              <div className="space-y-4 pt-1">
                    ...
                    <SelectContent>
                      <SelectItem value="home">
                        <span className="flex items-center gap-2">
                          <Home className="h-3.5 w-3.5" /> Came from Home
                        </span>
                      </SelectItem>
                      <SelectItem value="facility_referral">
                        <span className="flex items-center gap-2">
                          <Building2 className="h-3.5 w-3.5" /> Referred from Another Facility
                        </span>
                      </SelectItem>
                      <SelectItem value="self_referral">
                        <span className="flex items-center gap-2">
                          <Users className="h-3.5 w-3.5" /> Self Referral
                        </span>
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Textarea
                        placeholder={
                          referralSource === 'facility_referral'
                            ? 'e.g. Referred for specialist review — uncontrolled hypertension not responding to 1st line treatment…'
                            : 'e.g. Seeking second opinion for persistent lower abdominal pain…'
                        }
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select
                      value={priority}
                      onValueChange={(v) => setPriority(v as TriagePriority)}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      value={assignStaffId || 'none'}
                      onValueChange={(v) => setAssignStaffId(v === 'none' ? '' : v)}
                    >
                      <SelectTrigger>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Patient will be notified when called in and when results are ready.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <Select
                    value={addPaymentMethod || 'none'}
                    onValueChange={(v) => setAddPaymentMethod(v === 'none' ? '' : v)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select payment method" />
                    </SelectTrigger>
      ...
      <div className="flex items-center gap-2 mb-4 bg-blue-50 border border-blue-100 rounded-lg px-4 py-2.5 text-sm text-blue-700">
        ...
        <span>
          Patients are served in arrival order (FIFO). <strong>Urgent/Emergency cases</strong> are
          automatically moved to the top of the queue.
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 text-gray-400 gap-3">
          ...
          <p className="text-sm">
            No patients in queue for this date. Click <strong>Add Patient</strong> to begin.
          </p>
        </div>
      ) : (
            ...
            >
              <DialogContent className="max-w-sm">
                <DialogHeader>
                  ...
                  <DialogDescription className="sr-only">
                    Transfer this patient to another department.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 pt-1">
                  <p className="text-sm text-gray-600">
                    Transferring <strong>{entry.patientName}</strong> from{' '}
                    <em>{entry.department || 'current department'}</em>.
                  </p>
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-1.5 block">
                    ...
                    <Select value={transferDept} onValueChange={setTransferDept}>
                      ...
                      <SelectContent>
                        {DEPARTMENTS.filter((d) => d !== entry.department).map((d) => (
                          <SelectItem key={d} value={d}>
                            {d}
                          </SelectItem>
                        ))}
        ...
        (() => {
          ...
          return (
            <NursingDialog
              ...
              onPushBack={() => {
                setNurseOpen(null);
                handleStatusChange(entry, 'in-consultation');
              }}
        ...
        (() => {
          ...
          return (
            <TheatreDialog
              ...
              onSave={(e, data) => {
                updateMutation.mutate(
                  { id: e.id, data: { notes: data.notes, managementPlan: data.plan } as any },
                  {
                    onSuccess: () => {
                      setTheatreOpen(null);
                      invalidate();
        ...
        (() => {
          ...
          return (
            <ConsultationDialog
              ...
              onTransfer={() => {
                setConsultOpen(null);
                setTransferOpen(entry.id);
                setTransferDept(entry.department || '');
              }}
              onPushBack={() => {
                setConsultOpen(null);
                handleStatusChange(entry, 'waiting');
              }}
    ...
    <div className={`rounded-lg border-2 ${pcfg.cardBorder} ${pcfg.cardBg} p-3`}>
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-gray-400">
              🕐{' '}
              {new Date(entry.createdAt).toLocaleTimeString('en-UG', {
                hour: '2-digit',
                minute: '2-digit',
                hour12: true,
              })}
              {' · '}
              {new Date(entry.createdAt).toLocaleDateString('en-UG', {
                day: '2-digit',
                month: 'short',
              })}
            </p>
            ...
            {entry.referralSource && entry.referralSource !== 'home' && (
              <p className="text-xs text-purple-600 truncate">
                {entry.referralSource === 'facility_referral'
                  ? `🏨 Referred: ${entry.referralFacility || 'another facility'}`
                  : '↩ Self referral'}
              </p>
            )}
            {entry.notes && (
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <span
            className={`inline-flex items-center gap-1 text-xs font-semibold px-1.5 py-0.5 rounded-full ${pcfg.badge} ${pcfg.badgeBg}`}
          >
          ...
          {entry.notificationPhone && (
            <span className="inline-flex items-center gap-0.5 text-xs text-gray-400">
              ...
              {entry.notificationPhone.slice(-4)}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 mt-2.5 flex-wrap">
        ...
        {onConsult && (
          <Button
            ...
            disabled={isPending}
            title={
              entry.status === 'nursing'
                ? 'Record nursing care notes'
                : entry.status === 'theatre'
                  ? 'Theatre operative notes'
                  : 'Record vitals & management plan'
            }
          >
            {entry.status === 'nursing' ? (
...
const IMAGING_TESTS = [
  ...
  'Limb X-Ray (specify site)',
  'Wrist/Hand X-Ray',
  'Foot/Ankle X-Ray',
  'Abdominal Ultrasound',
  'Pelvic Ultrasound',
  'Obstetric Ultrasound (Dating/Anomaly)',
...
const LAB_PRICES: Record<string, number> = {
  ...
  HbA1c: 45000,
  'Urine Analysis': 15000,
  'Urine Culture & Sensitivity': 40000,
  'Stool MCS': 35000,
  'Liver Function Tests (LFTs)': 40000,
  'Renal Function Tests (RFTs)': 40000,
  Electrolytes: 35000,
  'Thyroid Function Tests (TFTs)': 50000,
  'HIV Test (ELISA/Rapid)': 20000,
  'Hepatitis B (HBsAg)': 25000,
  'Hepatitis C (Anti-HCV)': 25000,
  'RPR/VDRL (Syphilis)': 20000,
  'Widal Test': 20000,
  'Blood Culture & Sensitivity': 50000,
  'Sputum AFB (TB)': 25000,
  'GeneXpert MTB/RIF': 80000,
  'CD4 Count': 60000,
  'Viral Load': 150000,
  'Pregnancy Test (urine βHCG)': 15000,
  'Pap Smear': 40000,
  'Worm Ova & Cysts': 20000,
  'Lipid Profile': 50000,
  'Cardiac Enzymes (Troponin/CK-MB)': 80000,
  'INR/PT/APTT': 35000,
  ESR: 15000,
  CRP: 30000,
  'Urethral/Vaginal Swab MCS': 40000,
  'Blood Group & Cross Match': 30000,
};

const IMAGING_PRICES: Record<string, number> = {
  'Chest X-Ray (CXR) PA': 60000,
  'Chest X-Ray Lateral': 60000,
  'Abdominal X-Ray (AXR)': 50000,
  'Pelvic X-Ray': 50000,
  'Skull X-Ray': 55000,
  'Spine X-Ray (Cervical/Thoracic/Lumbar)': 60000,
  'Limb X-Ray (specify site)': 50000,
  'Wrist/Hand X-Ray': 50000,
  'Foot/Ankle X-Ray': 50000,
  'Abdominal Ultrasound': 80000,
  'Pelvic Ultrasound': 80000,
  'Obstetric Ultrasound (Dating/Anomaly)': 100000,
  'Renal/Bladder Ultrasound': 80000,
  'Thyroid Ultrasound': 80000,
  'Scrotal Ultrasound': 80000,
  'Breast Ultrasound': 80000,
  'Doppler Ultrasound (Vascular)': 120000,
  'Echocardiogram (ECHO)': 200000,
  'ECG (12-lead)': 40000,
  'CT Scan — Head': 500000,
  'CT Scan — Chest': 600000,
  'CT Scan — Abdomen/Pelvis': 600000,
  'MRI — Brain': 800000,
  'MRI — Spine': 800000,
  'MRI — Joint (specify)': 700000,
  Mammogram: 120000,
  'DEXA Scan (Bone Density)': 150000,
};
...
const NURSING_TX_LABELS: Record<string, string> = {
  ivFluids: 'IV Fluids',
  bloodTx: 'Blood Transfusion',
  woundDressing: 'Wound Dressing',
  oxygenTx: 'Oxygen Therapy',
  physio: 'Physiotherapy',
  immunisation: 'Immunisation',
};

type ConsultSaveData = {
  mgmtPlan: string;
  vitals: string;
  diagnosis: string;
  history: string;
  labInvest: string;
  imagingInvest: string;
  examFindings: string;
};
...
function ConsultationDialog({
  ...
  const [activeTab, setActiveTab] = useState<
    'triage' | 'history' | 'vitals' | 'diagnosis' | 'lab' | 'imaging' | 'plan' | 'procedures'
  >('triage');
      ...
      return res.json() as Promise<
        {
          ...
          interventionsPerformed: string | null;
  ...
  const setPrice = (key: string, val: number) =>
    setPriceOverrides((prev) => ({ ...prev, [key]: isNaN(val) ? 0 : val }));
  const getPrice = (key: string, def: number) =>
    priceOverrides[key] !== undefined ? priceOverrides[key] : def;

  // Vitals — parsed from saved snapshot
  const parsedVitals = React.useMemo(
    () => parseVitalsSnapshot(entry.vitalsSnapshot),
    [entry.vitalsSnapshot],
  );
  ...
  const [selectedLabTests, setSelectedLabTests] = useState<string[]>(() => {
    if (entry.labInvestigations) {
      try {
        return JSON.parse(entry.labInvestigations);
      } catch {
        return [];
      }
    }
  ...
  const [selectedImaging, setSelectedImaging] = useState<string[]>(() => {
    if (entry.imagingInvestigations) {
      try {
        return JSON.parse(entry.imagingInvestigations);
      } catch {
        return [];
      }
    }
  ...
  // Clinical decision (outpatient vs admit)
  const CONSULT_BASE = (import.meta.env.VITE_API_URL ?? import.meta.env.VITE_RENDER_URL ?? '') as string;
  const CONSULT_TOKEN = () => localStorage.getItem('medrise_admin_token') ?? '';
  const [admitDecision, setAdmitDecision] = useState<'outpatient' | 'admit' | null>(null);
  const [admitWard, setAdmitWard] = useState('General Ward');
  const [isAdmitting, setIsAdmitting] = useState(false);

  const toggleLab = (t: string) =>
    setSelectedLabTests((prev) => (prev.includes(t) ? prev.filter((x) => x !== t) : [...prev, t]));
  const toggleImaging = (t: string) =>
    setSelectedImaging((prev) => (prev.includes(t) ? prev.filter((x) => x !== t) : [...prev, t]));
  ...
  const buildSaveData = (): ConsultSaveData => {
    ...
    const historyNote = [
      ...
      .filter(Boolean)
      .join('\n');
    const diagOnly = [diagnosis, differentials && `DDx: ${differentials}`]
      .filter(Boolean)
      .join('\n');
    return {
      mgmtPlan,
  ...
  const handleSave = async () => {
    ...
    if (patient && (bp || temp || pulse || spo2 || weight || height || rr)) {
      createVitalsMutation.mutate(
        ...
        {
          onSuccess: () => {
            toast({ title: 'Vitals saved to EHR' });
    ...
    // Create admission record if doctor chose to admit
    if (admitDecision === 'admit' && entry.patientId) {
      setIsAdmitting(true);
      try {
        await fetch(`${CONSULT_BASE}/api/admissions`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${CONSULT_TOKEN()}` },
          body: JSON.stringify({
            patientId: entry.patientId,
            ...
            diagnosis: data.diagnosis || undefined,
            notes: data.mgmtPlan ? `Admitted from consultation. Plan: ${data.mgmtPlan}` : 'Admitted from consultation.',
          }),
        });
        toast({ title: `Patient admitted to ${admitWard}` });
      } catch {
        toast({ title: 'Admission record failed — please create manually', variant: 'destructive' });
      } finally {
    ...
    if (entry.patientId) {
      ...
      if (includeConsultFee)
        billingItems.push({
          description: 'Consultation Fee',
          quantity: 1,
          unitPrice: getPrice('__consultFee', 20000),
        });
      for (const p of selectedProcedures) {
        billingItems.push({
          description: p,
          quantity: 1,
          unitPrice: getPrice(p, PROCEDURES.find((x) => x.name === p)?.price ?? 0),
        });
      }
      if (customProcedure.trim())
        billingItems.push({
          description: customProcedure.trim(),
          quantity: 1,
          unitPrice: customProcedurePrice,
        });
      for (const t of selectedLabTests)
        billingItems.push({
          description: t,
          quantity: 1,
          unitPrice: getPrice(t, LAB_PRICES[t] ?? 0),
        });
      for (const t of selectedImaging)
        billingItems.push({
          description: t,
          quantity: 1,
          unitPrice: getPrice(t, IMAGING_PRICES[t] ?? 0),
        });
      if (billingItems.length > 0) {
        try {
          const res = await fetch(
            `${import.meta.env.VITE_RENDER_URL ?? ''}/api/billing/append-items`,
            {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
          ...
          if (res.ok) {
            toast({
              title: 'Bill updated',
              description: `${billingItems.length} item(s) added to patient account`,
            });
            qc.invalidateQueries();
          }
        } catch {
          toast({ title: 'Billing update failed — please add manually', variant: 'destructive' });
        }
      }
    }
  };
  ...
  const tabs: {
    id: 'triage' | 'history' | 'vitals' | 'diagnosis' | 'lab' | 'imaging' | 'plan' | 'procedures';
    label: string;
    icon: string;
  }[] = [
      ...
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-base">
            ...
            <span
              className={`ml-auto text-xs font-semibold px-2 py-0.5 rounded-full ${PRIORITY_CONFIG[entry.priority]?.badge} ${PRIORITY_CONFIG[entry.priority]?.badgeBg}`}
            >
              {PRIORITY_CONFIG[entry.priority]?.label}
            </span>
          </DialogTitle>
          <DialogDescription className="sr-only">
            Doctor's consultation panel for recording examination findings and treatment plan.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-wrap gap-2 text-xs bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
          {entry.department && (
            <span className="text-teal-700 font-medium">🏥 {entry.department}</span>
          )}
          {entry.referralSource && entry.referralSource !== 'home' && (
            <span className="text-purple-700">
              {entry.referralSource === 'facility_referral'
                ? `🏨 Referred from ${entry.referralFacility || 'facility'}`
                : '↩ Self Referral'}
            </span>
          )}
          {entry.notes && (
            <span className="text-gray-600 italic">
              CC: {entry.notes.split('--- Nursing Care ---')[0].trim()}
            </span>
          )}
        </div>
          ...
          {tabs.map((t) => (
            ...
            >
              ...
              {t.id === 'lab' && selectedLabTests.length > 0 && (
                <span className="bg-purple-100 text-purple-700 rounded-full px-1.5 py-0 text-xs">
                  {selectedLabTests.length}
                </span>
              )}
              {t.id === 'imaging' && selectedImaging.length > 0 && (
                <span className="bg-orange-100 text-orange-700 rounded-full px-1.5 py-0 text-xs">
                  {selectedImaging.length}
                </span>
              )}
              {t.id === 'procedures' && (selectedProcedures.length > 0 || includeConsultFee) && (
                <span className="bg-green-100 text-green-700 rounded-full px-1.5 py-0 text-xs">
                  {selectedProcedures.length + (includeConsultFee ? 1 : 0)}
                </span>
              )}
            </button>
          ))}
        ...
        <div className="pt-1">
          ...
          {activeTab === 'triage' && (
            <div className="space-y-3">
              <div className="bg-orange-50 border border-orange-200 rounded-lg px-3 py-2 text-xs text-orange-800">
                <strong>Read-only triage data recorded at point of entry.</strong> Use the other
                tabs to record your clinical findings.
              </div>
                ...
                <div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-3">
                    {entry.vitalsSnapshot.split(' | ').map((v, i) => (
                      <span
                        key={i}
                        className="inline-block mr-3 mb-1 bg-gray-50 border border-gray-200 rounded px-2 py-0.5 text-xs font-mono"
                      >
                        {v}
                      </span>
                    ))}
                    ...
                    <div key={t.id} className="border border-teal-200 rounded-lg overflow-hidden">
                      <div className="bg-teal-50 px-3 py-2 flex items-center justify-between">
                        <span className="text-xs font-semibold text-teal-800">
                          🩺 Triage Assessment{' '}
                          {triageRecords.length > 1 ? `#${triageRecords.length - idx}` : ''}
                          {t.assignedNurseName ? ` — by ${t.assignedNurseName}` : ''}
                        </span>
                        <span className="text-xs text-teal-600">
                          {new Date(t.triageTime).toLocaleString('en-UG', {
                            dateStyle: 'short',
                            timeStyle: 'short',
                          })}
                        </span>
                      ...
                      <div className="p-3 space-y-2.5 bg-white">
                        ...
                        {(t.bloodPressure ||
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {t.bloodPressure && (
                                <span className="text-xs bg-gray-50 border border-gray-200 rounded px-2 py-0.5 font-mono">
                                  BP: {t.bloodPressure}
                                </span>
                              )}
                              {t.pulseRate && (
                              ...
                              {t.oxygenSaturation && (
                                <span className="text-xs bg-gray-50 border border-gray-200 rounded px-2 py-0.5 font-mono">
                                  SpO₂: {t.oxygenSaturation}
                                </span>
                              )}
                              {t.respiratoryRate && (
                                <span className="text-xs bg-gray-50 border border-gray-200 rounded px-2 py-0.5 font-mono">
                                  RR: {t.respiratoryRate}
                                </span>
                              )}
                              {t.weight && (
                                <span className="text-xs bg-gray-50 border border-gray-200 rounded px-2 py-0.5 font-mono">
                              ...
                              {t.painScale != null && (
                                <span className="text-xs bg-red-50 border border-red-200 rounded px-2 py-0.5 font-mono text-red-700">
                                  Pain: {t.painScale}/10
                                </span>
                              )}
                            </div>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-800 bg-blue-50 border border-blue-100 rounded p-2 italic">
                            {t.chiefComplaint}
                          </p>
                        </div>
                        ...
                        {t.nursingAssessment && (
                          <div>
                            ...
                            <p className="text-sm text-gray-800 bg-teal-50 border border-teal-100 rounded p-2 whitespace-pre-wrap">
                              {t.nursingAssessment}
                            </p>
                          </div>
                        )}
                        ...
                        {t.interventionsPerformed && (
                          <div>
                            ...
                            <p className="text-sm text-gray-800 bg-amber-50 border border-amber-100 rounded p-2 whitespace-pre-wrap">
                              {t.interventionsPerformed}
                            </p>
                          </div>
                        )}
                        ...
                        {t.reassessmentNotes && (
                          <div>
                            ...
                            <p className="text-sm text-gray-800 bg-purple-50 border border-purple-100 rounded p-2 whitespace-pre-wrap">
                              {t.reassessmentNotes}
                            </p>
                          </div>
                        )}
                        ...
                        {t.isEmergency && (
                          <div className="inline-flex items-center gap-1 bg-red-100 text-red-700 text-xs font-bold px-2 py-1 rounded-full">
                            🚨 Emergency
                          </div>
                        )}
                      </div>
                ...
                : !entry.vitalsSnapshot &&
                  !entry.managementPlan && (
                    <div className="text-center py-6 text-gray-400">
                      ...
                      <p className="text-sm">
                        No formal triage assessment on record for this patient.
                      </p>
                    </div>
                  )}
              ...
              {entry.managementPlan && (
                <div>
                  <p className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">
                    💊 Emergency Treatment at Entry
                  </p>
                  <div className="bg-red-50 border border-red-100 rounded-lg p-3 text-sm text-gray-700 whitespace-pre-wrap">
                    {entry.managementPlan}
                  </div>
                </div>
              )}
                ...
                (() => {
                  try {
                    ...
                    return Array.isArray(labs) && labs.length > 0 ? (
                      <div>
                        <p className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">
                          🔬 Lab Tests Requested at Triage
                        </p>
                        <div className="flex flex-wrap gap-1.5">
                          {labs.map((t: string) => (
                            <span
                              key={t}
                              className="text-xs bg-purple-50 border border-purple-200 text-purple-700 rounded-full px-2 py-0.5"
                            >
                              {t}
                            </span>
                          ))}
                ...
                (() => {
                  try {
                    ...
                    return Array.isArray(imgs) && imgs.length > 0 ? (
                      <div>
                        <p className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">
                          🩻 Imaging Requested at Triage
                        </p>
                        <div className="flex flex-wrap gap-1.5">
                          {imgs.map((t: string) => (
                            <span
                              key={t}
                              className="text-xs bg-orange-50 border border-orange-200 text-orange-700 rounded-full px-2 py-0.5"
                            >
                              {t}
                            </span>
                          ))}
          ...
          {activeTab === 'vitals' && (
            <div className="space-y-3">
              <p className="text-xs text-gray-500 mb-2">
                Record observations at time of consultation.
              </p>
              <div className="grid grid-cols-2 gap-3">
                {[
              ...
              {!entry.patientId && (
                <p className="text-xs text-amber-600 bg-amber-50 rounded p-2">
                  ⚠ Walk-in patient — vitals saved to queue only. Register in Patient Database to
                  link to EHR.
                </p>
              )}
            </div>
          ...
          {activeTab === 'diagnosis' && (
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 rounded-lg p-3 text-xs text-blue-700">
                <strong>Tip:</strong> Use ICD-10 codes if available (e.g. B54 – Unspecified malaria,
                K59.1 – Functional diarrhoea). This helps with HMIS reporting.
              </div>
            </div>
          )}
          ...
          {activeTab === 'lab' && (
            <div className="space-y-3">
              <p className="text-xs text-gray-500">
                Select required lab tests. Prices are pre-filled from the price list — edit each one
                if this patient's rate differs.
              </p>
              <div className="grid grid-cols-1 gap-1.5 max-h-64 overflow-y-auto pr-1">
                {LAB_TESTS.map((t) => {
                  ...
                  return (
                    <div
                      key={t}
                      className={`flex items-center gap-2 text-xs px-2.5 py-1.5 rounded-lg border transition-colors ${selected ? 'border-purple-400 bg-purple-50 text-purple-800 font-medium' : 'border-gray-200 text-gray-700'}`}
                    >
              ...
              <div>
                <label className="text-xs text-gray-500 block mb-1">
                  Additional tests / clinical notes for lab
                </label>
                <Input
                  placeholder="e.g. Urea & Creatinine, Amylase, custom panel…"
                  value={labNotes}
              ...
              {selectedLabTests.length > 0 && (
                <div className="bg-purple-50 rounded-lg p-2.5 text-xs text-purple-700 flex items-start justify-between gap-2">
                  <span>
                    <strong>{selectedLabTests.length} test(s) requested:</strong>{' '}
                  ...
                  <button
                    ...
                    onClick={() =>
                      printInvestigationRequest('lab', entry, {
                        lab: labNotes
                          ? [...selectedLabTests, `Note: ${labNotes}`]
                          : selectedLabTests,
                      })
                    }
                  >
          ...
          {activeTab === 'imaging' && (
            <div className="space-y-3">
              <p className="text-xs text-gray-500">
                Select required radiology / imaging studies. Edit the price if this patient's rate
              ...
              <div className="grid grid-cols-1 gap-1.5 max-h-64 overflow-y-auto pr-1">
                {IMAGING_TESTS.map((t) => {
                  ...
                  return (
                    <div
                      key={t}
                      className={`flex items-center gap-2 text-xs px-2.5 py-1.5 rounded-lg border transition-colors ${selected ? 'border-orange-400 bg-orange-50 text-orange-800 font-medium' : 'border-gray-200 text-gray-700'}`}
                    >
                      <label className="flex items-center gap-2 flex-1 cursor-pointer min-w-0">
                        <input
                          type="checkbox"
              ...
              <div>
                <label className="text-xs text-gray-500 block mb-1">
                  Additional notes for radiology
                </label>
                <Input
                  placeholder="e.g. Contrast CT, specific views, clinical query…"
                  value={imagingNotes}
              ...
              {selectedImaging.length > 0 && (
                <div className="bg-orange-50 rounded-lg p-2.5 text-xs text-orange-700 flex items-start justify-between gap-2">
                  <span>
                    <strong>{selectedImaging.length} study(ies) requested:</strong>{' '}
                    {selectedImaging.join(', ')}
                  </span>
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      printInvestigationRequest('imaging', entry, {
                        imaging: imagingNotes
                          ? [...selectedImaging, `Note: ${imagingNotes}`]
                          : selectedImaging,
                      })
                    }
                  >
          ...
          {activeTab === 'procedures' && (
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-100 rounded-lg px-3 py-2 text-xs text-green-800">
                <strong>Auto-Billing:</strong> Checked items are automatically added to this
                patient's bill when you click "Save Consultation". Lab and Imaging selections from
                their respective tabs are also included.
              </div>
              {!entry.patientId && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-700">
                  ⚠ Walk-in patient not registered. Register in Patient Database to enable
                  auto-billing.
                </div>
              )}
              <div>
                <div
                  className={`flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg border-2 ${includeConsultFee ? 'border-teal-400 bg-teal-50 text-teal-800' : 'border-gray-200 text-gray-600'}`}
                >
                  <label className="flex items-center gap-2 flex-1 cursor-pointer">
                    <input
                      type="checkbox"
                      className="accent-teal-600 w-4 h-4"
                      checked={includeConsultFee}
                      onChange={(e) => setIncludeConsultFee(e.target.checked)}
                    />
                  ...
                  {!includeConsultFee && (
                    <span className="text-sm font-mono font-bold opacity-40">20,000</span>
                  )}
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-1 gap-1.5 max-h-52 overflow-y-auto pr-1">
                  {PROCEDURES.map((p) => {
                    ...
                    return (
                      <div
                        key={p.name}
                        className={`flex items-center gap-2 text-xs px-2.5 py-1.5 rounded-lg border transition-colors ${selected ? 'border-indigo-400 bg-indigo-50 text-indigo-800 font-medium' : 'border-gray-200 text-gray-700'}`}
                      >
                        <label className="flex items-center gap-2 flex-1 cursor-pointer min-w-0">
                          <input
                            type="checkbox"
              ...
              <div>
                <label className="text-xs text-gray-500 block mb-1">
                  Custom Procedure / Other Charge
                </label>
                <div className="flex gap-2">
                  <Input
                    placeholder="e.g. Suture removal, Colposcopy, other service…"
                    ...
                    <span className="font-mono text-gray-800">
                      ...
                      {(
                        ...
                        selectedProcedures.reduce(
                          (s, p) =>
                            s + getPrice(p, PROCEDURES.find((x) => x.name === p)?.price || 0),
                          0,
                        ) +
                    ...
                    {includeConsultFee && (
                      <div className="flex items-center justify-between px-3 py-1.5 gap-2">
                        ...
                        <span className="font-mono text-teal-700">
                          {getPrice('__consultFee', 20000).toLocaleString()}
                        </span>
                      </div>
                    )}
                    {selectedProcedures.map((p) => (
                      <div key={p} className="flex items-center justify-between px-3 py-1.5 gap-2">
                        ...
                        <span className="font-mono text-indigo-700">
                          {getPrice(
                            p,
                            PROCEDURES.find((x) => x.name === p)?.price || 0,
                          ).toLocaleString()}
                        </span>
                      </div>
                    ))}
                      ...
                      <div className="flex items-center justify-between px-3 py-1.5 gap-2">
                        ...
                        <span className="font-mono text-gray-600">
                          {customProcedurePrice ? (
                            customProcedurePrice.toLocaleString()
                          ) : (
                    ...
                    {selectedLabTests.map((t) => (
                      <div key={t} className="flex items-center justify-between px-3 py-1.5 gap-2">
                        ...
                        <span className="font-mono text-purple-700">
                          {getPrice(t, LAB_PRICES[t] || 0).toLocaleString()}
                        </span>
                      </div>
                    ))}
                    {selectedImaging.map((t) => (
                      <div key={t} className="flex items-center justify-between px-3 py-1.5 gap-2">
                        ...
                        <span className="font-mono text-orange-700">
                          {getPrice(t, IMAGING_PRICES[t] || 0).toLocaleString()}
                        </span>
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-3">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setAdmitDecision('outpatient')}
                    className={`p-3 rounded-lg border-2 text-sm font-medium transition-colors ${admitDecision === 'outpatient' ? 'bg-green-50 border-green-500 text-green-700' : 'border-gray-200 text-gray-600 hover:border-green-300'}`}
                  >
                    🏠 Manage As Outpatient
                  </button>
                  <button
                    type="button"
                    onClick={() => setAdmitDecision('admit')}
                    className={`p-3 rounded-lg border-2 text-sm font-medium transition-colors ${admitDecision === 'admit' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-blue-300'}`}
                  >
                    🏥 Admit Patient
                  </button>
                </div>
                {admitDecision === 'admit' && (
                  <div className="space-y-2">
                    <div>
                      <label className="text-xs font-medium text-gray-600 block mb-1">Ward / Destination *</label>
                      <select
                        value={admitWard}
                      ...
                      >
                        {['General Ward','Medical Ward','Surgical Ward','Maternity Ward','Paediatrics Ward','ICU','HDU','Isolation Ward','Dental Ward','Eye Ward','Orthopaedic Ward'].map(w => (
                          <option key={w}>{w}</option>
                        ))}
                      </select>
                    </div>
                    <p className="text-xs text-blue-600 bg-blue-50 px-2 py-1.5 rounded">
                      ℹ️ An admission record will be created for <strong>{admitWard}</strong> on save.
                    </p>
                  </div>
                )}
                {admitDecision === 'outpatient' && (
                  <p className="text-xs text-green-600 bg-green-50 px-2 py-1.5 rounded">
                    ✓ Patient will be managed as outpatient and discharged from queue.
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="flex gap-2 pt-3 border-t border-gray-100 mt-2 flex-wrap items-center">
          ...
          <div className="flex gap-2 flex-1 flex-wrap">
            ...
            <Button
              className="flex-1 min-w-[100px] bg-teal-600 hover:bg-teal-700 text-white"
              onClick={handleSave}
              disabled={isSaving}
            >
              {isSaving || isAdmitting ? 'Saving…' : admitDecision === 'admit' ? '🏥 Save & Admit' : 'Save Consultation'}
            </Button>
          </div>
        </div>
      </DialogContent>
...
function NursingDialog({
  ...
  const [nursingPriceOverrides, setNursingPriceOverrides] = useState<Record<string, number>>({
    ...NURSING_TX_PRICES,
  });
  const setNursingPrice = (key: string, val: number) =>
    setNursingPriceOverrides((prev) => ({ ...prev, [key]: isNaN(val) ? 0 : val }));
  ...
  const removeRow = (id: string) =>
    setTreatments((rows) => (rows.length > 1 ? rows.filter((r) => r.id !== id) : rows));
  ...
  const removeMon = (id: string) =>
    setMonitoring((rows) => (rows.length > 1 ? rows.filter((r) => r.id !== id) : rows));
    ...
    >
      <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">
            Nursing care notes and treatment administration record.
          </DialogDescription>
        </DialogHeader>
        ...
        {(entry.vitalsSnapshot || entry.managementPlan) && (
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 space-y-1.5">
            ...
            {entry.vitalsSnapshot && (
              <div className="flex flex-wrap gap-1">
                {entry.vitalsSnapshot.split(' | ').map((v, i) => (
                  <span
                    key={i}
                    className="text-xs bg-white border border-orange-200 rounded px-2 py-0.5 font-mono"
                  >
                    {v}
                  </span>
                ))}
            ...
            {entry.managementPlan && (
              <p className="text-xs text-orange-900 whitespace-pre-wrap">{entry.managementPlan}</p>
            )}
          </div>
        )}
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <Button
                ...
                variant="outline"
                onClick={addMon}
                className="h-7 text-xs px-2 border-teal-400 text-teal-700 hover:bg-teal-50"
              >
                + Add Observation
              </Button>
            </div>
              ...
              <div className="min-w-[760px]">
                <div className="grid grid-cols-[70px_1fr_70px_70px_70px_70px_60px_1fr_80px_32px] bg-teal-700 text-white text-[10px] font-semibold">
                  {[
                    ...
                    <div key={i} className="px-1.5 py-2">
                      {h}
                    </div>
                  ))}
                </div>
                {monitoring.map((row, idx) => (
                  <div
                    key={row.id}
                    className={`grid grid-cols-[70px_1fr_70px_70px_70px_70px_60px_1fr_80px_32px] border-t border-teal-100 ${idx % 2 === 0 ? 'bg-white' : 'bg-teal-50/30'}`}
                  >
                    <div className="px-1 py-1">
                      <Input
                        type="time"
                        value={row.time}
                        onChange={(e) => updateMon(row.id, 'time', e.target.value)}
                        className="h-7 text-[10px] border-gray-200 px-1"
                      />
                    </div>
                    <div className="px-1 py-1">
                      <Input
                        placeholder="120/80"
                        value={row.bp}
                        onChange={(e) => updateMon(row.id, 'bp', e.target.value)}
                        className="h-7 text-[10px] border-gray-200 px-1"
                      />
                    </div>
                    <div className="px-1 py-1">
                      <Input
                        placeholder="72"
                        value={row.pulse}
                        onChange={(e) => updateMon(row.id, 'pulse', e.target.value)}
                        className="h-7 text-[10px] border-gray-200 px-1"
                      />
                    </div>
                    <div className="px-1 py-1">
                      <Input
                        placeholder="36.5"
                        value={row.temp}
                        onChange={(e) => updateMon(row.id, 'temp', e.target.value)}
                        className="h-7 text-[10px] border-gray-200 px-1"
                      />
                    </div>
                    <div className="px-1 py-1">
                      <Input
                        placeholder="98"
                        value={row.spo2}
                        onChange={(e) => updateMon(row.id, 'spo2', e.target.value)}
                        className="h-7 text-[10px] border-gray-200 px-1"
                      />
                    </div>
                    <div className="px-1 py-1">
                      <Input
                        placeholder="16"
                        value={row.rr}
                        onChange={(e) => updateMon(row.id, 'rr', e.target.value)}
                        className="h-7 text-[10px] border-gray-200 px-1"
                      />
                    </div>
                    <div className="px-1 py-1">
                      <Input
                        placeholder="0–10"
                        value={row.pain}
                        onChange={(e) => updateMon(row.id, 'pain', e.target.value)}
                        className="h-7 text-[10px] border-gray-200 px-1"
                      />
                    </div>
                    <div className="px-1 py-1">
                      <Select value={row.avpu} onValueChange={(v) => updateMon(row.id, 'avpu', v)}>
                        <SelectTrigger className="h-7 text-[10px] border-gray-200 px-1">
                          <SelectValue placeholder="AVPU" />
                        </SelectTrigger>
                        <SelectContent>
                          {AVPU_OPTIONS.map((o) => (
                    ...
                    <div className="px-1 py-1">
                      <Input
                        placeholder="ml"
                        value={row.urineOut}
                        onChange={(e) => updateMon(row.id, 'urineOut', e.target.value)}
                        className="h-7 text-[10px] border-gray-200 px-1"
                      />
                    </div>
            ...
            <p className="text-[10px] text-gray-400 mt-1">
              Record observations at regular intervals. AVPU — Alert / Voice / Pain / Unresponsive.
            </p>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <Button
                ...
                variant="outline"
                onClick={addRow}
                className="h-7 text-xs px-2 border-pink-300 text-pink-700 hover:bg-pink-50"
              >
                + Add Treatment
              </Button>
            </div>
            <div className="border border-gray-200 rounded-lg overflow-x-auto">
              <div className="min-w-[720px]">
                ...
                {treatments.map((row, idx) => (
                  <div
                    key={row.id}
                    className={`grid grid-cols-[2fr_1fr_1fr_1fr_1.5fr_1.5fr_1.5fr_32px] border-t border-gray-100 ${idx % 2 === 0 ? 'bg-white' : 'bg-pink-50/30'}`}
                  >
                    <div className="px-1.5 py-1.5">
                      <Input
                        placeholder="e.g. Paracetamol, NS 0.9%…"
                        value={row.treatment}
                    ...
                    <div className="px-1.5 py-1.5">
                      <Select
                        value={row.route}
                        onValueChange={(v) => updateRow(row.id, 'route', v)}
                      >
                    ...
                    <div className="px-1.5 py-1.5">
                      <Select
                        value={row.givenBy}
                        onValueChange={(v) => updateRow(row.id, 'givenBy', v)}
                      >
                        <SelectTrigger className="h-8 text-xs border-gray-200">
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="_other" className="text-xs italic text-gray-400">
                            Other…
                          </SelectItem>
                        </SelectContent>
                      </Select>
                      {row.givenBy === '_other' && (
                        <Input
                          ...
                          className="h-7 text-xs mt-1 border-gray-200"
                          onChange={(e) => updateRow(row.id, 'givenBy', e.target.value)}
                        />
                      )}
                    </div>
              ...
              <div className="border border-blue-200 rounded-lg overflow-hidden">
                <div className="flex items-center justify-between gap-2 px-3 py-2 bg-blue-50">
                  <label className="flex items-center gap-2 flex-1 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={stx.ivFluids}
                      onChange={(e) => stxSet('ivFluids', e.target.checked)}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <input
                      ...
                      min="0"
                      step="500"
                      value={nursingPriceOverrides.ivFluids ?? NURSING_TX_PRICES.ivFluids}
                      onChange={(e) => setNursingPrice('ivFluids', parseFloat(e.target.value))}
                      className="w-24 h-6 text-xs text-right font-mono bg-white border border-blue-300 rounded px-1 focus:outline-none focus:ring-1 focus:ring-blue-400"
                    />
                  </div>
                </div>
                {stx.ivFluids && (
                  <div className="grid grid-cols-2 gap-2 p-3 bg-white">
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Fluid Type</p>
                      <Select
                        value={stx.ivFluidType}
                        onValueChange={(v) => stxSet('ivFluidType', v)}
                      >
                        <SelectTrigger className="h-8 text-xs">
                          <SelectValue placeholder="Select fluid" />
                        </SelectTrigger>
                        <SelectContent>
                          {IV_FLUID_TYPES.map((f) => (
                    ...
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Volume (ml)</p>
                      <Input
                        placeholder="e.g. 500"
                        value={stx.ivFluidVol}
                        onChange={(e) => stxSet('ivFluidVol', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Rate (ml/hr)</p>
                      <Input
                        placeholder="e.g. 125"
                        value={stx.ivFluidRate}
                        onChange={(e) => stxSet('ivFluidRate', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">IV Site</p>
                      <Input
                        placeholder="e.g. Left antecubital"
                        value={stx.ivFluidSite}
                        onChange={(e) => stxSet('ivFluidSite', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div className="col-span-2">
                      <p className="text-[10px] text-gray-500 mb-0.5">Given By</p>
                      <Select
                        value={stx.ivFluidGivenBy}
                        onValueChange={(v) => stxSet('ivFluidGivenBy', v)}
                      >
                        <SelectTrigger className="h-8 text-xs">
                ...
                <div className="flex items-center justify-between gap-2 px-3 py-2 bg-red-50">
                  <label className="flex items-center gap-2 flex-1 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={stx.bloodTx}
                      onChange={(e) => stxSet('bloodTx', e.target.checked)}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <input
                      ...
                      min="0"
                      step="500"
                      value={nursingPriceOverrides.bloodTx ?? NURSING_TX_PRICES.bloodTx}
                      onChange={(e) => setNursingPrice('bloodTx', parseFloat(e.target.value))}
                      className="w-24 h-6 text-xs text-right font-mono bg-white border border-red-300 rounded px-1 focus:outline-none focus:ring-1 focus:ring-red-400"
                    />
                  </div>
                </div>
                {stx.bloodTx && (
                  <div className="grid grid-cols-2 gap-2 p-3 bg-white">
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Blood Type / Product</p>
                      <Input
                        placeholder="e.g. O+ Whole Blood, PRBC"
                        value={stx.bloodType}
                        onChange={(e) => stxSet('bloodType', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Units</p>
                      <Input
                        placeholder="e.g. 1"
                        value={stx.bloodUnits}
                        onChange={(e) => stxSet('bloodUnits', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div className="col-span-2">
                      <p className="text-[10px] text-gray-500 mb-0.5">Reactions / Observations</p>
                      <Input
                        placeholder="e.g. None, or describe any reaction"
                        value={stx.bloodReactions}
                        onChange={(e) => stxSet('bloodReactions', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div className="col-span-2">
                      <p className="text-[10px] text-gray-500 mb-0.5">Given By</p>
                      <Select
                        value={stx.bloodGivenBy}
                        onValueChange={(v) => stxSet('bloodGivenBy', v)}
                      >
                        <SelectTrigger className="h-8 text-xs">
                          <SelectValue placeholder="Staff name" />
                        </SelectTrigger>
              ...
              <div className="border border-amber-200 rounded-lg overflow-hidden">
                <div className="flex items-center justify-between gap-2 px-3 py-2 bg-amber-50">
                  <label className="flex items-center gap-2 flex-1 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={stx.woundDressing}
                      onChange={(e) => stxSet('woundDressing', e.target.checked)}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <input
                      ...
                      min="0"
                      step="500"
                      value={nursingPriceOverrides.woundDressing ?? NURSING_TX_PRICES.woundDressing}
                      onChange={(e) => setNursingPrice('woundDressing', parseFloat(e.target.value))}
                      className="w-24 h-6 text-xs text-right font-mono bg-white border border-amber-300 rounded px-1 focus:outline-none focus:ring-1 focus:ring-amber-400"
                    />
                  </div>
                </div>
                {stx.woundDressing && (
                  <div className="grid grid-cols-2 gap-2 p-3 bg-white">
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Wound Site</p>
                      <Input
                        placeholder="e.g. Right shin"
                        value={stx.woundSite}
                        onChange={(e) => stxSet('woundSite', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Dressing Type</p>
                      <Input
                        placeholder="e.g. Non-adherent, Betadine gauze"
                        value={stx.dressingType}
                        onChange={(e) => stxSet('dressingType', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div className="col-span-2">
                      <p className="text-[10px] text-gray-500 mb-0.5">
                        Wound Description / Condition
                      </p>
                      <Input
                        placeholder="e.g. Clean, granulating, 3cm × 2cm, no slough"
                        value={stx.woundDesc}
                        onChange={(e) => stxSet('woundDesc', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div className="col-span-2">
                      <p className="text-[10px] text-gray-500 mb-0.5">Done By</p>
                      <Select
                        value={stx.woundDoneBy}
                        onValueChange={(v) => stxSet('woundDoneBy', v)}
                      >
                        <SelectTrigger className="h-8 text-xs">
                          <SelectValue placeholder="Staff name" />
                        </SelectTrigger>
              ...
              <div className="border border-sky-200 rounded-lg overflow-hidden">
                <div className="flex items-center justify-between gap-2 px-3 py-2 bg-sky-50">
                  <label className="flex items-center gap-2 flex-1 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={stx.oxygenTx}
                      onChange={(e) => stxSet('oxygenTx', e.target.checked)}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <input
                      ...
                      min="0"
                      step="500"
                      value={nursingPriceOverrides.oxygenTx ?? NURSING_TX_PRICES.oxygenTx}
                      onChange={(e) => setNursingPrice('oxygenTx', parseFloat(e.target.value))}
                      className="w-24 h-6 text-xs text-right font-mono bg-white border border-sky-300 rounded px-1 focus:outline-none focus:ring-1 focus:ring-sky-400"
                    />
                  </div>
                </div>
                {stx.oxygenTx && (
                  <div className="grid grid-cols-2 gap-2 p-3 bg-white">
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Delivery Device</p>
                    ...
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Flow Rate (L/min)</p>
                      <Input
                        placeholder="e.g. 4"
                        value={stx.o2Rate}
                        onChange={(e) => stxSet('o2Rate', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Target SpO₂ (%)</p>
                      <Input
                        placeholder="e.g. ≥95%"
                        value={stx.o2SpO2Target}
                        onChange={(e) => stxSet('o2SpO2Target', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                ...
                <div className="flex items-center justify-between gap-2 px-3 py-2 bg-green-50">
                  <label className="flex items-center gap-2 flex-1 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={stx.physio}
                      onChange={(e) => stxSet('physio', e.target.checked)}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <input
                      ...
                      min="0"
                      step="500"
                      value={nursingPriceOverrides.physio ?? NURSING_TX_PRICES.physio}
                      onChange={(e) => setNursingPrice('physio', parseFloat(e.target.value))}
                      className="w-24 h-6 text-xs text-right font-mono bg-white border border-green-300 rounded px-1 focus:outline-none focus:ring-1 focus:ring-green-400"
                    />
                  </div>
                </div>
                {stx.physio && (
                  <div className="grid grid-cols-2 gap-2 p-3 bg-white">
                    ...
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Duration (min)</p>
                      <Input
                        placeholder="e.g. 20"
                        value={stx.physioDuration}
                        onChange={(e) => stxSet('physioDuration', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div className="col-span-2">
                      <p className="text-[10px] text-gray-500 mb-0.5">Patient Response</p>
                      <Input
                        placeholder="e.g. Tolerated well, improved chest clearance"
                        value={stx.physioResponse}
                        onChange={(e) => stxSet('physioResponse', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div className="col-span-2">
                      <p className="text-[10px] text-gray-500 mb-0.5">Done By</p>
                      <Select
                        value={stx.physioDoneBy}
                        onValueChange={(v) => stxSet('physioDoneBy', v)}
                      >
                        <SelectTrigger className="h-8 text-xs">
                          <SelectValue placeholder="Staff name" />
                        </SelectTrigger>
              ...
              <div className="border border-purple-200 rounded-lg overflow-hidden">
                <div className="flex items-center justify-between gap-2 px-3 py-2 bg-purple-50">
                  <label className="flex items-center gap-2 flex-1 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={stx.immunisation}
                      onChange={(e) => stxSet('immunisation', e.target.checked)}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <input
                      ...
                      min="0"
                      step="500"
                      value={nursingPriceOverrides.immunisation ?? NURSING_TX_PRICES.immunisation}
                      onChange={(e) => setNursingPrice('immunisation', parseFloat(e.target.value))}
                      className="w-24 h-6 text-xs text-right font-mono bg-white border border-purple-300 rounded px-1 focus:outline-none focus:ring-1 focus:ring-purple-400"
                    />
                  </div>
                </div>
                {stx.immunisation && (
                  <div className="grid grid-cols-2 gap-2 p-3 bg-white">
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Vaccine Name</p>
                      <Input
                        placeholder="e.g. Tetanus, MMR, Hepatitis B"
                        value={stx.vaccineName}
                        onChange={(e) => stxSet('vaccineName', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Dose</p>
                      <Input
                        placeholder="e.g. 0.5ml, 1st dose"
                        value={stx.vaccineDose}
                        onChange={(e) => stxSet('vaccineDose', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Injection Site</p>
                      <Input
                        placeholder="e.g. Left deltoid"
                        value={stx.vaccineSite}
                        onChange={(e) => stxSet('vaccineSite', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Batch Number</p>
                      <Input
                        placeholder="e.g. BN20240512"
                        value={stx.vaccineBatch}
                        onChange={(e) => stxSet('vaccineBatch', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Next Due Date</p>
                      <Input
                        type="date"
                        value={stx.vaccineNextDue}
                        onChange={(e) => stxSet('vaccineNextDue', e.target.value)}
                        className="h-8 text-xs"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-0.5">Given By</p>
                      <Select
                        value={stx.vaccineGivenBy}
                        onValueChange={(v) => stxSet('vaccineGivenBy', v)}
                      >
                        <SelectTrigger className="h-8 text-xs">
                          <SelectValue placeholder="Staff name" />
                        </SelectTrigger>
          ...
          <div>
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="px-3 py-2 font-mono">
                      {netBalance !== null ? (
                        <span
                          className={`font-bold text-base ${netBalance >= 0 ? 'text-blue-700' : 'text-red-700'}`}
                        >
                          {netBalance >= 0 ? '+' : ''}
                          {netBalance} ml
                        </span>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Positive = fluid retention · Negative = fluid deficit
            </p>
          </div>
        </div>
...
// ─── Theatre Dialog ───────────────────────────────────────────────────────────
function parseTheatreNotes(notes: string | null | undefined) {
  const ex = (label: string) => {
    const m = notes?.match(new RegExp(`^${label}:\\s*(.+)`, 'm'));
    return m?.[1]?.trim() ?? '';
  };
  return {
    preDiag: ex('Diagnosis'),
    prePlan: ex('Planned Procedure'),
    preConsent: ex('Consent'),
    preAllergies: ex('Allergies'),
    preNPO: ex('NPO since'),
    preIV: ex('IV Access'),
    preBloods: ex('Pre-op bloods'),
    preMeds: ex('Pre-op meds'),
    preAnaesType: ex('Anaesthesia type'),
    preSite: ex('Operative site'),
    preSurgeonNotes: ex('Surgeon notes'),
    intraSurgeon: ex('Surgeon'),
    intraAnaes: ex('Anaesthetist'),
    intraScrubNurse: ex('Scrub Nurse'),
    intraCircNurse: ex('Circulating Nurse'),
    intraProcedure: ex('Procedure performed'),
    intraFindings: ex('Operative findings'),
    intraDuration: ex('Duration'),
    intraBloodLoss: ex('Estimated blood loss').replace(' mL', ''),
    intraFluidsGiven: ex('IV fluids given'),
    intraSpecimen: ex('Specimen sent'),
    intraDrains: ex('Drains'),
    intraSwabs: ex('Swab/instrument count'),
    intraComplications: ex('Intra-op complications'),
    postDx: ex('Post-op diagnosis'),
    postWound: ex('Wound status'),
    postPlan: ex('Post-op plan'),
    postMeds: ex('Medications'),
    postDiet: ex('Diet orders'),
    postActivity: ex('Activity'),
    postFollowUp: ex('Follow-up'),
    postNotes: ex('Notes'),
    recVitals: ex('Vitals in recovery'),
    recAldrete: ex('Aldrete score'),
    recO2: ex('O₂ therapy'),
    recPain: ex('Pain score'),
    recNausea: ex('Nausea/vomiting'),
    recNurse: ex('Recovery nurse'),
    recDischarge: ex('Discharged from recovery'),
    recNotes: ex('Recovery notes'),
  };
}
...
function TheatreDialog({
  ...
  const p = React.useMemo(() => parseTheatreNotes(entry.notes), [entry.notes]);

  // Preoperative
  const [preConsent, setPreConsent] = useState(p.preConsent);
  const [preAllergies, setPreAllergies] = useState(p.preAllergies);
  const [preNPO, setPreNPO] = useState(p.preNPO);
  const [preIV, setPreIV] = useState(p.preIV);
  const [preBloods, setPreBloods] = useState(p.preBloods);
  const [preMeds, setPreMeds] = useState(p.preMeds);
  const [preAnaesType, setPreAnaesType] = useState(p.preAnaesType);
  const [preSite, setPreSite] = useState(p.preSite);
  const [preDiag, setPreDiag] = useState(p.preDiag);
  const [prePlan, setPrePlan] = useState(p.prePlan);
  const [preSurgeonNotes, setPreSurgeonNotes] = useState(p.preSurgeonNotes);

  // Intraoperative
  const [intraFindings, setIntraFindings] = useState(p.intraFindings);
  const [intraProcedure, setIntraProcedure] = useState(p.intraProcedure);
  const [intraScrubNurse, setIntraScrubNurse] = useState(p.intraScrubNurse);
  const [intraCircNurse, setIntraCircNurse] = useState(p.intraCircNurse);
  const [intraAnaes, setIntraAnaes] = useState(p.intraAnaes);
  const [intraSurgeon, setIntraSurgeon] = useState(p.intraSurgeon);
  const [intraSpecimen, setIntraSpecimen] = useState(p.intraSpecimen);
  const [intraDrains, setIntraDrains] = useState(p.intraDrains);
  const [intraComplications, setIntraComplications] = useState(p.intraComplications);
  const [intraSwabs, setIntraSwabs] = useState(p.intraSwabs);
  const [intraDuration, setIntraDuration] = useState(p.intraDuration);
  const [intraBloodLoss, setIntraBloodLoss] = useState(p.intraBloodLoss);
  const [intraFluidsGiven, setIntraFluidsGiven] = useState(p.intraFluidsGiven);

  // Postoperative
  const [postDx, setPostDx] = useState(p.postDx);
  const [postPlan, setPostPlan] = useState(p.postPlan);
  const [postWound, setPostWound] = useState(p.postWound);
  const [postMeds, setPostMeds] = useState(p.postMeds);
  const [postDiet, setPostDiet] = useState(p.postDiet);
  const [postActivity, setPostActivity] = useState(p.postActivity);
  const [postFollowUp, setPostFollowUp] = useState(p.postFollowUp);
  const [postNotes, setPostNotes] = useState(p.postNotes);

  // Recovery Room
  const [recVitals, setRecVitals] = useState(p.recVitals);
  const [recAldrete, setRecAldrete] = useState(p.recAldrete);
  const [recO2, setRecO2] = useState(p.recO2);
  const [recPain, setRecPain] = useState(p.recPain);
  const [recNausea, setRecNausea] = useState(p.recNausea);
  const [recDischarge, setRecDischarge] = useState(p.recDischarge);
  const [recNurse, setRecNurse] = useState(p.recNurse);
  const [recNotes, setRecNotes] = useState(p.recNotes);
      ...
      <DialogContent className="max-w-2xl max-h-[94vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            Patient: <strong>{entry.patientName}</strong> · #{entry.arrivalOrder} ·{' '}
            {entry.department}
          </DialogDescription>
        </DialogHeader>
          ...
          {tab === 'preop' && (
            <div className="space-y-3">
              ...
              {[
                {
                  ...
                  val: preDiag,
                  set: setPreDiag,
                  ph: 'e.g. Acute appendicitis',
                },
                {
                  label: 'Planned Procedure',
                  val: prePlan,
                  set: setPrePlan,
                  ph: 'e.g. Appendicectomy (open)',
                },
                {
                  label: 'Anaesthesia Type',
                  val: preAnaesType,
                  set: setPreAnaesType,
                  ph: 'e.g. General anaesthesia (GA), Spinal, LA',
                },
                {
                  label: 'Operative Site / Mark',
                  val: preSite,
                  set: setPreSite,
                  ph: 'e.g. Right iliac fossa, marked & confirmed',
                },
              ].map((f) => (
                <div key={f.label}>
                  <label className="text-xs text-gray-600 font-medium block mb-0.5">
                    {f.label}
                  </label>
                  <Input
                    placeholder={f.ph}
                ...
                {[
                  {
                    ...
                    val: preConsent,
                    set: setPreConsent,
                    ph: 'e.g. Written consent signed by patient',
                  },
                  {
                    label: 'Known Allergies',
                    val: preAllergies,
                    set: setPreAllergies,
                    ph: 'e.g. Penicillin — rash',
                  },
                  {
                    label: 'NPO since',
                    val: preNPO,
                    set: setPreNPO,
                    ph: 'e.g. 22:00 (solids), 00:00 (liquids)',
                  },
                  {
                    label: 'IV Access',
                    val: preIV,
                    set: setPreIV,
                    ph: 'e.g. 18G IV cannula, right antecubital',
                  },
                  {
                    label: 'Pre-op bloods / investigations',
                    val: preBloods,
                    set: setPreBloods,
                    ph: 'e.g. FBC, U&E, Group & X-match done',
                  },
                  {
                    label: 'Pre-operative medications',
                    val: preMeds,
                    set: setPreMeds,
                    ph: 'e.g. Cefazolin 1g IV prophylaxis given 30 min pre-op',
                  },
                ].map((f) => (
                  <div key={f.label}>
                    <label className="text-xs text-gray-600 font-medium block mb-0.5">
              ...
              <div>
                ...
                <Textarea
                  placeholder="Additional notes, concerns, or patient-specific considerations…"
                  value={preSurgeonNotes}
                  onChange={(e) => setPreSurgeonNotes(e.target.value)}
                  className="min-h-[60px] resize-none text-sm"
                />
              </div>
            </div>
          ...
          {tab === 'intraop' && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  },
                  {
                    label: 'Scrub Nurse',
                    val: intraScrubNurse,
                    set: setIntraScrubNurse,
                    ph: 'Name',
                  },
                  {
                    label: 'Circulating Nurse',
                    val: intraCircNurse,
                    set: setIntraCircNurse,
                    ph: 'Name',
                  },
                  {
                    label: 'Duration (min)',
                    val: intraDuration,
                    set: setIntraDuration,
                    ph: 'e.g. 75',
                  },
                  {
              ...
              <div>
                ...
                <Input
                  placeholder="e.g. Laparotomy — appendicectomy"
                  value={intraProcedure}
                  onChange={(e) => setIntraProcedure(e.target.value)}
                  className="h-8 text-sm"
                />
              </div>
              <div>
                ...
                <Textarea
                  placeholder="e.g. Inflamed, perforated appendix at the tip. Purulent fluid in RIF. No free bowel contents…"
                  value={intraFindings}
                  onChange={(e) => setIntraFindings(e.target.value)}
                  className="min-h-[70px] resize-none text-sm"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  {
                    ...
                    val: intraBloodLoss,
                    set: setIntraBloodLoss,
                    ph: 'e.g. 150',
                  },
                  {
                    label: 'IV Fluids Given',
                    val: intraFluidsGiven,
                    set: setIntraFluidsGiven,
                    ph: 'e.g. NS 1L + RL 500mL',
                  },
                  {
                    label: 'Specimen sent (histology/culture)',
                    val: intraSpecimen,
                    set: setIntraSpecimen,
                    ph: 'e.g. Appendix → histology',
                  },
                  {
                    label: 'Drains inserted',
                    val: intraDrains,
                    set: setIntraDrains,
                    ph: 'e.g. None / Corrugated drain RIF',
                  },
                ].map((f) => (
                  <div key={f.label}>
                    <label className="text-xs text-gray-600 font-medium block mb-0.5">
              ...
              <div>
                ...
                <Input
                  placeholder="e.g. None / Bleeding from mesoappendix — controlled with diathermy"
                  value={intraComplications}
                  onChange={(e) => setIntraComplications(e.target.value)}
                  className="h-8 text-sm"
                />
              </div>
            </div>
          ...
          {tab === 'postop' && (
            <div className="space-y-3">
              ...
              <div>
                ...
                <Input
                  placeholder="e.g. Perforated appendicitis"
                  value={postDx}
                  onChange={(e) => setPostDx(e.target.value)}
                  className="h-8 text-sm"
                />
              </div>
              <div>
                ...
                <Textarea
                  placeholder="e.g. Continue IV antibiotics (Ceftriaxone + Metronidazole) for 5 days, wound dressing daily, drain removal at 48h, DVT prophylaxis, early mobilisation…"
                  value={postPlan}
                  onChange={(e) => setPostPlan(e.target.value)}
                  className="min-h-[70px] resize-none text-sm"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  {
                    ...
                    val: postWound,
                    set: setPostWound,
                    ph: 'e.g. Closed — sutures in situ, dry',
                  },
                  {
                    label: 'Medications / analgesia',
                    val: postMeds,
                    set: setPostMeds,
                    ph: 'e.g. Paracetamol 1g TDS, Diclofenac 75mg BD',
                  },
                  {
                    label: 'Diet orders',
                    val: postDiet,
                    set: setPostDiet,
                    ph: 'e.g. Nil oral until bowel sounds, then sips',
                  },
                  {
                    label: 'Activity / Mobilisation',
                    val: postActivity,
                    set: setPostActivity,
                    ph: 'e.g. Bed rest 24h, then assisted ambulation',
                  },
                  {
                    label: 'Follow-up appointment',
                    val: postFollowUp,
                    set: setPostFollowUp,
                    ph: 'e.g. Review in clinic at 1 week / wound clinic',
                  },
                ].map((f) => (
                  <div key={f.label}>
                    <label className="text-xs text-gray-600 font-medium block mb-0.5">
              ...
              <div>
                <label className="text-xs text-gray-600 font-medium block mb-0.5">
                  Additional Post-op Notes
                </label>
                <Textarea
                  placeholder="Any other orders, family communication, ward instructions…"
                  value={postNotes}
          ...
          {tab === 'recovery' && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  {
                    label: 'Vitals on arrival to recovery',
                    val: recVitals,
                    set: setRecVitals,
                    ph: 'e.g. BP 118/76, HR 82, SpO₂ 98%, T 36.8°C',
                  },
                  { label: 'Aldrete Score', val: recAldrete, set: setRecAldrete, ph: 'e.g. 9/10' },
                  {
                    ...
                    val: recO2,
                    set: setRecO2,
                    ph: 'e.g. 2L/min via nasal prongs',
                  },
                  {
                    label: 'Pain score (NRS 0-10)',
                    val: recPain,
                    set: setRecPain,
                    ph: 'e.g. 4/10 — morphine 2mg IV given',
                  },
                  {
                    label: 'Nausea / vomiting',
                    val: recNausea,
                    set: setRecNausea,
                    ph: 'e.g. Mild nausea — ondansetron 4mg given',
                  },
                  { label: 'Recovery Nurse', val: recNurse, set: setRecNurse, ph: 'Name' },
                  {
                    label: 'Discharged from recovery at',
                    val: recDischarge,
                    set: setRecDischarge,
                    ph: 'e.g. 14:35 — to surgical ward',
                  },
                ].map((f) => (
                  <div key={f.label}>
                    <label className="text-xs text-gray-600 font-medium block mb-0.5">
              ...
              <div>
                ...
                <Textarea
                  placeholder="Any events, observations or special instructions during recovery…"
                  value={recNotes}
