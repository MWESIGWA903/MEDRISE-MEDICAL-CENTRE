...
export default function BillingTab() {
  ...
  function handleAddInvoice(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (items.some((i) => !i.description)) {
      toast({ title: 'Fill all item descriptions', variant: 'destructive' });
      return;
    }
  ...
  function handlePayment(e: React.FormEvent<HTMLFormElement>) {
    ...
    updateMutation.mutate(
      { id: payOpen.id, data: { paidAmount, paymentMethod, status } },
      {
        onSuccess: () => {
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Manage patient bills, payments and revenue tracking.
          </p>
        </div>
              ...
              <div>
                ...
                <div className="mt-3 text-right">
                  ...
                  <p className="text-xl font-bold text-primary">
                    UGX {total.toLocaleString('en-UG', { minimumFractionDigits: 2 })}
                  </p>
                </div>
              </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <Card className="border-0 bg-green-50">
            <CardContent className="p-4">
              <p className="text-xs text-green-600 font-medium mb-1">Revenue Collected</p>
              <p className="text-lg font-bold text-green-900">
                UGX {parseFloat(stats.totalPaid).toLocaleString()}
              </p>
            </CardContent>
          </Card>
          <Card className="border-0 bg-red-50">
            <CardContent className="p-4">
              ...
              <p className="text-lg font-bold text-red-900">
                UGX {parseFloat(stats.totalUnpaid).toLocaleString()}
              </p>
            </CardContent>
          </Card>
          <Card className="border-0 bg-purple-50">
            <CardContent className="p-4">
              ...
              <p className="text-lg font-bold text-purple-900">
                UGX {parseFloat(stats.thisMonthRevenue).toLocaleString()}
              </p>
            </CardContent>
          </Card>
        </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          <Badge
                            variant="outline"
                            className={`text-xs capitalize shrink-0 ${STATUS_COLORS[inv.status] ?? ''}`}
                          >
                            {inv.status}
                          </Badge>
                        </div>
                        <div className="flex items-center gap-4 text-sm text-gray-600 flex-wrap">
                          ...
                          <span>
                            ...
                            <strong className="text-green-700">
                              UGX {parseFloat(inv.paidAmount).toLocaleString()}
                            </strong>
                          </span>
                          {balance > 0 && (
                                  ...
                                  deleteMutation.mutate(
                                    { id: inv.id },
                                    {
                                      onSuccess: () => {
                                        ...
                                        qc.invalidateQueries({
                                          queryKey: getGetBillingStatsQueryKey(),
                                        });
                                      },
                                    },
                                  )
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {lineItems.map((item, idx) => {
                            ...
                            const deptColor = dept
                              ...
                              : '';
                            const descBody = dept
                              ? String(item.description).replace(`[${dept}] `, '')
                              : item.description;
                              ...
                              <tr key={idx} className="hover:bg-white">
                                <td className="px-4 py-2">
                                  <div className="flex items-center gap-2">
                                    {dept && (
                                      <span
                                        className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${deptColor}`}
                                      >
                                        {dept}
                                      </span>
                                    )}
                        ...
                        <tfoot>
                          <tr className="border-t-2 border-gray-200 bg-white">
                            ...
                            <td className="px-3 py-2 text-right font-mono font-bold text-gray-900">
                              UGX {parseFloat(inv.totalAmount).toLocaleString()}
                            </td>
                          </tr>
                        </tfoot>
                      </table>
            ...
            <form onSubmit={handlePayment} className="space-y-4 pt-2">
              <div>
                <p className="text-sm text-gray-600 mb-3">
                  Invoice total: <strong>UGX {parseFloat(payOpen.total).toLocaleString()}</strong>
                </p>
              ...
              <div>
                ...
                >
                  ...
                  <option value="mtn-mobile-money">
                    MTN Mobile Money — 0770 775268 / 0751 527730
                  </option>
                ...
                <p className="text-xs text-gray-400 mt-1.5">
                  Mobile Money: Send to <strong>0770 775268</strong> or <strong>0751 527730</strong>{' '}
