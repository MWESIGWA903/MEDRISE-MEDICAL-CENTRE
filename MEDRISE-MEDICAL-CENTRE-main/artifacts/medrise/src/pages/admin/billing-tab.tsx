...
import { Plus, Trash2, Receipt, TrendingUp, DollarSign, Clock, CheckCircle2, X, ChevronDown, ChevronRight } from "lucide-react";
    ...
    <div>
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Dialog open={addOpen} onOpenChange={setAddOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>Create New Invoice</DialogTitle><DialogDescription className="sr-only">Create a billing invoice for a patient.</DialogDescription></DialogHeader>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filtered.map(inv => {
            const lineItems: { description: string; quantity: number; unitPrice: string | number }[] = inv.items ?? [];
            const isExpanded = expandedInvoice === inv.id;
            const balance = parseFloat(inv.totalAmount) - parseFloat(inv.paidAmount);
            return (
              <Card key={inv.id} className="border border-gray-100 overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          <span className="font-semibold text-gray-900 truncate">{inv.patientName ?? `Patient #${inv.patientId}`}</span>
                          <Badge variant="outline" className={`text-xs capitalize shrink-0 ${STATUS_COLORS[inv.status] ?? ""}`}>{inv.status}</Badge>
                        </div>
                        <div className="flex items-center gap-4 text-sm text-gray-600 flex-wrap">
                          ...
                          {balance > 0 && <span className="text-red-600 text-xs font-medium">Balance: UGX {balance.toLocaleString()}</span>}
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          {lineItems.length > 0 && (
                            <button onClick={() => setExpandedInvoice(isExpanded ? null : inv.id)} className="flex items-center gap-0.5 text-xs text-primary hover:underline">
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {lineItems.map((item, idx) => {
                            const dept = String(item.description).match(/^\[([^\]]+)\]/)?.[1];
                            const deptColors: Record<string, string> = { Consultation: "bg-teal-100 text-teal-700", Laboratory: "bg-purple-100 text-purple-700", Radiology: "bg-orange-100 text-orange-700", Nursing: "bg-blue-100 text-blue-700" };
                            const deptColor = dept ? (deptColors[dept] ?? "bg-gray-100 text-gray-600") : "";
                            const descBody = dept ? String(item.description).replace(`[${dept}] `, "") : item.description;
                              ...
                              <tr key={idx} className="hover:bg-white">
                                <td className="px-4 py-2">
                                  <div className="flex items-center gap-2">
                                    {dept && <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${deptColor}`}>{dept}</span>}
                                    <span className="text-gray-800">{descBody}</span>
                                  </div>
                                </td>
                                <td className="px-3 py-2 text-center text-gray-600">{qty}</td>
                                <td className="px-3 py-2 text-right font-mono text-gray-600">{unitPrice.toLocaleString()}</td>
                                <td className="px-3 py-2 text-right font-mono font-semibold text-gray-800">{(unitPrice * qty).toLocaleString()}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                        <tfoot>
                          <tr className="border-t-2 border-gray-200 bg-white">
                            ...
                            <td className="px-3 py-2 text-right font-mono font-bold text-gray-900">UGX {parseFloat(inv.totalAmount).toLocaleString()}</td>
                          </tr>
                        </tfoot>
                      </table>
                    </div>
      ...
      {payOpen && (
        <Dialog open={!!payOpen} onOpenChange={() => setPayOpen(null)}>
          <DialogContent className="max-w-sm">
            <DialogHeader><DialogTitle>Record Payment</DialogTitle><DialogDescription className="sr-only">Record a payment received for an invoice.</DialogDescription></DialogHeader>
