...
export default function FeedbackTab() {
  ...
  const filtered =
    filter === 'all' ? feedbackList : feedbackList.filter((f) => f.rating === filter);
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm">
          Reviews and ratings submitted by patients about services received.
        </p>
      </div>
          ...
          <CardContent className="p-5 text-center">
            ...
            <div className="flex justify-center mb-1">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star
                  key={s}
                  className={`h-4 w-4 ${Number(avgRating) >= s ? 'fill-yellow-400 text-yellow-400' : 'text-gray-200'}`}
                />
              ))}
            </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((f) => (
            <Card
              key={f.id}
              className="border shadow-none hover:border-primary/30 transition-colors"
            >
              <CardContent className="p-5">
                ...
                <div className="flex items-center justify-between text-xs text-gray-400 pt-2 border-t border-gray-100">
                  ...
                  {f.wouldRecommend && (
                    <span
                      className={`font-medium ${f.wouldRecommend === 'yes' ? 'text-green-600' : f.wouldRecommend === 'no' ? 'text-red-500' : 'text-gray-500'}`}
                    >
