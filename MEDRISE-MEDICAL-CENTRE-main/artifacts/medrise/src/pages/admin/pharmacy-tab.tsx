...
import {
  ...
  useUpdatePharmacyStock,
  useDeletePharmacyStock,
  useDispensePharmacy,
  useGetPharmacyStats,
  getListPharmacyStockQueryKey,
...
export default function PharmacyTab() {
  ...
  const { data: stock = [], isLoading } = useListPharmacyStock(
    { search: search || undefined, lowStock: showLowStock || undefined } as any,
    { query: {} as any },
  );
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  function handleDispense(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (qty > dispenseOpen.qty) {
      toast({ title: 'Insufficient stock', variant: 'destructive' });
      return;
    }
    dispenseMutation.mutate(
      { data: { stockId: dispenseOpen.id, quantity: qty, notes: String(fd.get('notes') || '') } },
      {
        onSuccess: () => {
        ...
        onError: (err: Error) =>
          toast({ title: err.message || 'Failed to dispense', variant: 'destructive' }),
      },
    );
  }
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            name="category"
            defaultValue={String(defaultValues?.category ?? '')}
        ...
        <div>
          ...
          <Input
            name="unit"
            defaultValue={String(defaultValues?.unit ?? 'units')}
        ...
        <div>
          ...
          <Input
            ...
            type="number"
            min="0"
            defaultValue={String(defaultValues?.quantity ?? '0')}
            required
          />
        </div>
        <div>
          ...
          <Input
            ...
            type="number"
            min="0"
            defaultValue={String(defaultValues?.reorderLevel ?? '10')}
          />
        </div>
        <div>
          ...
          <Input
            name="expiryDate"
            type="date"
            defaultValue={String(defaultValues?.expiryDate ?? '')}
          />
        </div>
        <div>
          ...
          <Input
            ...
            min="0"
            step="0.01"
            defaultValue={String(defaultValues?.buyingPrice ?? '')}
          />
        </div>
        <div>
          ...
          <Input
            ...
            min="0"
            step="0.01"
            defaultValue={String(defaultValues?.sellingPrice ?? '')}
          />
        </div>
      </div>
    </div>
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Track medicines, stock levels, expiry dates and dispensing.
          </p>
        </div>
        <Dialog open={addOpen} onOpenChange={setAddOpen}>
          ...
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Add Drug to Stock</DialogTitle>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <Card className="border-0 bg-orange-50">
            <CardContent className="p-4">
              <p className="text-xs text-orange-600 font-medium mb-1">Expiring (30d)</p>
              <p className="text-2xl font-bold text-orange-900">{stats.expiringItems}</p>
            </CardContent>
          </Card>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {stock.map((s) => {
                ...
                const soon =
                  s.expiryDate &&
                  s.expiryDate <= new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10);
                const expired = s.expiryDate && s.expiryDate < now;
                return (
                  <tr
                    key={s.id}
                    className={`${isOut ? 'bg-red-50/30' : isLow ? 'bg-yellow-50/30' : ''}`}
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <Badge
                        variant="outline"
                        className={`text-xs ${isOut ? 'bg-red-50 text-red-700 border-red-200' : isLow ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : 'bg-green-50 text-green-700 border-green-200'}`}
                      >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        <Button
                          ...
                          onClick={() =>
                            setDispenseOpen({ id: s.id, name: s.drugName, qty: s.quantity })
                          }
                          disabled={isOut}
                        >
                        ...
                        <Dialog
                          ...
                          onOpenChange={(open) => setEditItem(open ? { id: s.id } : null)}
                        >
                          <DialogTrigger asChild>
                            <Button
                              variant="ghost"
                          ...
                          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                            <DialogHeader>
                              <DialogTitle>Edit — {s.drugName}</DialogTitle>
                              <DialogDescription className="sr-only">
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                This will permanently remove this drug from inventory.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                    ...
                                    { id: s.id },
                                    {
                                      onSuccess: () => {
                                        ...
                                        qc.invalidateQueries({
                                          queryKey: getListPharmacyStockQueryKey(),
                                        });
                                        qc.invalidateQueries({
                                          queryKey: getGetPharmacyStatsQueryKey(),
                                        });
                                      },
                                    },
                                  )
      ...
      {dispenseOpen && (
        <Dialog open={!!dispenseOpen} onOpenChange={() => setDispenseOpen(null)}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle>Dispense — {dispenseOpen.name}</DialogTitle>
              <DialogDescription className="sr-only">
            ...
            <form onSubmit={handleDispense} className="space-y-4 pt-2">
              <p className="text-sm text-gray-600">
                Available: <strong>{dispenseOpen.qty} units</strong>
              </p>
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-1">
              ...
              <div className="flex justify-end gap-3">
                <Button type="button" variant="outline" onClick={() => setDispenseOpen(null)}>
                  Cancel
                </Button>
