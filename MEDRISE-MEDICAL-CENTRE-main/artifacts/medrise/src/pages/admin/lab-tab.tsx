...
export default function LabTab({ adminId }: { adminId?: number }) {
  ...
  function handleAddOrder(e: React.FormEvent<HTMLFormElement>) {
    ...
    const patientId = parseInt(addOrderPatientId, 10);
    if (!patientId) {
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Order tests, track processing status, and record results.
          </p>
        </div>
              ...
              <form onSubmit={handleAddOrder} className="space-y-4 pt-2">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    ...
                    <PatientCombobox
                      patients={patients}
                      value={addOrderPatientId}
                      onValueChange={setAddOrderPatientId}
                    />
                  </div>
                  <div>
              ...
              <CardContent className="p-0">
                <div className="flex items-start justify-between p-4">
                  <div className="flex-1">
                    ...
                    {order.clinicalInfo && (
                      <p className="text-xs text-gray-500 mt-0.5">{order.clinicalInfo}</p>
                    )}
                  ...
                  <div className="flex items-center gap-2 ml-4">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          updateOrderMutation.mutate(
                            { id: order.id, data: { status: 'in-progress' } },
                            {
                              onSuccess: () => {
                    ...
                    {(order.status === 'pending' || order.status === 'in-progress') && (
                      <Button
                        ...
                        onClick={() =>
                          setResultOpen({ orderId: order.id, testName: order.testName })
                        }
                      >
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            This will also delete any recorded results.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                          ...
                          {r.referenceRange && (
                            <div>
                              ...
                              <p className="text-gray-700">{r.referenceRange}</p>
                            </div>
                          )}
                          {r.interpretation && (
                            <div>
      ...
      {resultOpen && (
        <Dialog open={!!resultOpen} onOpenChange={() => setResultOpen(null)}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Record Result — {resultOpen.testName}</DialogTitle>
              <DialogDescription className="sr-only">
                Enter the laboratory test result for this order.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleRecordResult} className="space-y-4 pt-2">
              ...
              <div className="flex justify-end gap-3 pt-2">
                <Button type="button" variant="outline" onClick={() => setResultOpen(null)}>
                  Cancel
                </Button>
                <Button type="submit" disabled={createResultMutation.isPending}>
                  {createResultMutation.isPending ? 'Saving...' : 'Submit Result'}
                </Button>
              </div>
            </form>
          </DialogContent>
