...
export default function PatientPortal() {
  ...
  const { data: consultations = [] } = useListConsultations({ patientId: pid } as any, {
    query: { enabled: !!pid } as any,
  });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: vitals = [] } = useListVitals({ patientId: pid } as any, {
    query: { enabled: !!pid } as any,
  });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: invoices = [] } = useListInvoices({ patientId: pid } as any, {
    query: { enabled: !!pid } as any,
  });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: labOrders = [] } = useListLabOrders({ patientId: pid } as any, {
    query: { enabled: !!pid } as any,
  });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const myAppointments = appointments.filter(
    (a) =>
      patientSession &&
      (a.phone === patientSession.phone ||
        a.patientName.toLowerCase().includes(patientSession.name.split(' ')[0].toLowerCase())),
  );
            ...
            {navItems.map(({ tab, label, icon: Icon, count }) => (
              ...
              >
                ...
                {typeof count === 'number' && count > 0 && (
                  <span className="ml-1 bg-gray-100 text-gray-600 rounded-full text-xs px-1.5 py-0.5">
                    {count}
                  </span>
                )}
              </button>
              ...
              {[
                {
                  ...
                  icon: Stethoscope,
                  tab: 'consultations' as PortalTab,
                  color: 'text-teal-600',
                },
                {
                ...
                },
                {
                  ...
                  icon: Calendar,
                  tab: 'appointments' as PortalTab,
                  color: 'text-purple-600',
                },
                {
                  label: 'Outstanding Bills',
                  value: invoices.filter((i) => i.status !== 'paid' && i.status !== 'cancelled')
                    .length,
                  icon: Receipt,
                  tab: 'billing' as PortalTab,
                  color: 'text-red-600',
                ...
                <Card
                  key={s.label}
                  className="border border-gray-100 cursor-pointer hover:border-primary/30 transition-colors"
              ...
              <Card className="border border-gray-100 mb-4">
                ...
                <CardContent>
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Date:</span> {consultations[0].visitDate}
                  </p>
                  {consultations[0].diagnosis && (
                    <p className="text-sm text-gray-600 mt-1">
                      <span className="font-medium">Diagnosis:</span> {consultations[0].diagnosis}
                    </p>
                  )}
                  {consultations[0].followUpDate && (
                    <p className="text-sm text-primary mt-1 font-medium">
                      Follow-up: {consultations[0].followUpDate}
                    </p>
                  )}
                </CardContent>
              </Card>
                    ...
                    <CardContent className="p-4">
                      ...
                      {expanded === c.id && (
                        <div className="mt-3 pt-3 border-t border-gray-50 space-y-2 text-sm">
                          ...
                          {c.followUpDate && (
                            <div>
                              <p className="text-xs font-medium text-gray-500">Follow-up</p>
                              <p className="text-primary font-medium">{c.followUpDate}</p>
                            </div>
                          )}
                          {c.notes && (
                            <div>
        ...
        {activeTab === 'billing' && (
          <div>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-700">
                            Total:{' '}
                            <strong>UGX {parseFloat(inv.totalAmount).toLocaleString()}</strong>
                          </p>
                          {parseFloat(inv.paidAmount) > 0 && (
                            <p className="text-sm text-green-700">
                              Paid: UGX {parseFloat(inv.paidAmount).toLocaleString()}
                            </p>
                          )}
                          {inv.status !== 'paid' && inv.status !== 'cancelled' && (
                            <p className="text-sm text-red-600 font-medium">
                              Balance: UGX{' '}
                              {(
                                parseFloat(inv.totalAmount) - parseFloat(inv.paidAmount)
                              ).toLocaleString()}
                            </p>
                          )}
            ...
            <div className="mt-6 p-4 bg-blue-50 rounded-xl text-sm text-blue-800">
              ...
              <p>
                Visit us at MEDRISE MEDICAL CENTRE or call{' '}
                <a href="tel:+256770775268" className="font-semibold">
                  +256 770 775268
                </a>{' '}
                  ...
                  <Card key={order.id} className="border border-gray-100">
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <Badge
                              variant="outline"
                              className={`text-xs ${order.status === 'completed' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'}`}
                            >
                              {order.status}
                            </Badge>
                          </div>
                      ...
                      {order.status === 'pending' && (
                        <p className="text-xs text-yellow-600 mt-2 bg-yellow-50 rounded px-2 py-1">
