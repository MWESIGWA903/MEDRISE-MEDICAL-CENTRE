...
import {
  ArrowRight, Clock, Shield, Award, Users,
  Stethoscope, Heart, Scissors, Activity, Baby,
  ScanLine, FlaskConical, Pill, Smile, Mic2, ClipboardList,
} from "lucide-react";
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto mb-14">
            <span className="inline-block text-xs font-bold uppercase tracking-widest text-secondary mb-3">What We Offer</span>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Our Services</h2>
            <p className="text-gray-600">
              MEDRISE Medical Centre provides {DEPARTMENTS.length} professional healthcare services under one roof, staffed by trained specialists dedicated to your health.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {DEPARTMENTS.map((dept) => {
              ...
              return (
                ...
                >
                  <div className={`h-14 w-14 rounded-2xl ${colors.bg} ${colors.text} flex items-center justify-center mb-4`}>
                    {DEPT_ICONS[dept.icon]}
                  </div>
                  <h3 className="font-bold text-gray-900 text-sm leading-tight mb-2">{dept.name}</h3>
                  <p className="text-xs text-gray-500 leading-relaxed line-clamp-3">{dept.description}</p>
                </div>
              );
            })}
          </div>
