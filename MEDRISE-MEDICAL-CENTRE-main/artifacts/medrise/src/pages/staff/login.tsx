...
import { Stethoscope, Loader2, ArrowLeft, KeyRound, AlertTriangle } from "lucide-react";
import logoBannerPath from "@assets/medrise_logo_banner.jpg";

const MEDICAL_ROLES = [
  "medical_director", "doctor", "nurse", "midwife", "receptionist",
  "pharmacist", "lab_technician", "billing_officer", "records_officer", "staff", "owner", "admin",
];
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12">
              ...
              <FormField control={form.control} name="password" render={({ field }) => (
                <FormItem>
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => { setResetOpen(true); setResetStep("request"); }} className="text-xs text-teal-600 hover:underline">Forgot password?</button>
                  </div>
                  <FormControl><Input type="password" placeholder="Enter your password" className="h-12" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <Button type="submit" className="w-full h-12 text-base bg-teal-600 hover:bg-teal-700 text-white mt-4 rounded-xl" disabled={loginMutation.isPending}>
                {loginMutation.isPending ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Authenticating...</> : "Sign In to Staff Portal"}
              </Button>
            </form>
          </Form>

          <div className="mt-6 pt-6 border-t border-gray-100 text-center">
            <p className="text-sm text-gray-500">Are you an administrator?{" "}
              <Link href="/admin/login" className="text-primary font-medium hover:underline">Use Admin Login</Link>
            </p>
          </div>
        </div>
      </div>
      ...
      <Dialog open={resetOpen} onOpenChange={open => { setResetOpen(open); if (!open) { setResetStep("request"); setDebugToken(null); } }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><KeyRound className="h-5 w-5 text-teal-600" /> Reset Password</DialogTitle>
          ...
          {resetStep === "request" ? (
            <Form {...resetRequestForm}>
              <form onSubmit={resetRequestForm.handleSubmit(onResetRequest)} className="space-y-4">
                ...
                <Button type="submit" className="w-full bg-teal-600 hover:bg-teal-700" disabled={resetPending}>
