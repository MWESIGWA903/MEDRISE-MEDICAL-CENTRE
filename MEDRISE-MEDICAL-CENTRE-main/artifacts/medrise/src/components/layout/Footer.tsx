      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {DEPARTMENTS.map(d => (
                <li key={d.id} className="text-primary-foreground/80">{d.name}</li>
              ))}
            </ul>
          </div>
            ...
            <ul className="space-y-4">
              ...
              <li className="flex items-start gap-3">
                ...
                <div className="flex flex-col gap-1">
                  <a href={`tel:${CONTACT_INFO.phoneMTNRaw}`} className="text-primary-foreground/80 hover:text-white transition-colors text-sm">
                    {CONTACT_INFO.phoneMTN} <span className="text-xs text-primary-foreground/50">(MTN — Calls Only)</span>
                  </a>
                  <a href={`tel:${CONTACT_INFO.phoneAirtelRaw}`} className="text-primary-foreground/80 hover:text-white transition-colors text-sm">
                    {CONTACT_INFO.phoneAirtel} <span className="text-xs text-primary-foreground/50">(Airtel — WhatsApp & Calls)</span>
                  </a>
                </div>
              </li>
        ...
        <div className="border-t border-primary-foreground/20 pt-8 mt-8 flex flex-col md:flex-row items-center justify-between text-sm text-primary-foreground/60">
          ...
          <div className="flex gap-4 mt-2 md:mt-0">
            ...
            <span>|</span>
            <Link href="/privacy#disclaimer" className="hover:text-white transition-colors">Medical Disclaimer</Link>
            <span>|</span>
